...
namespace WebImcopa.control
{
    ...
    public class ADHelper 
    {
        ...
        //private static string ADUser = Configuration.ADAdminUser ;
        //private static string ADPassword = Configuration.ADAdminPassword ;
        private static string ADPath = ConfigurationManager.AppSettings["ADPath"].ToString();
        private static string ADUser = Decrypt(ConfigurationManager.AppSettings["ADAdminUser"].ToString());
        private static string ADPassword = Decrypt(ConfigurationManager.AppSettings["ADAdminPassword"].ToString());
        #endregion
        ...
        public static void SetUserPassword(string UserName, string OldPassword, string NewPassword, out Exception ex)
        {
            ex = null;
            try
            {
                UserPrincipal usr = isExistsUserNameAD(UserName, OldPassword);
                if (usr != null)
                {
                    usr.ChangePassword(OldPassword, NewPassword);
                    usr.Save();
                    //var ou = ConfigurationManager.AppSettings["ADOu"];
                    //var path = String.Format("{0}/CN={1},{2},{3}", ADPath, usr.Name, ou, GetLDAPDomain());                    
                    //var dominio =  ConfigurationManager.AppSettings["ADDomain"] + "\\";
                    //DirectoryEntry directoryEntry = new DirectoryEntry(path, dominio + UserName, OldPassword, AuthenticationTypes.Secure);
        ...
        public static bool ResetPassword(string UserName, out Exception ex)
        {
            ex = null;
            try
            {
                UserPrincipal usr = isExistsUserNameAD(UserName);

                if (usr != null)
                {
                    usr.Enabled = true;
                    usr.SetPassword(Decrypt(ConfigurationManager.AppSettings["ADDefaultPassword"]));
                    usr.Save();
        ...
        public static UserPrincipal isExistsUserNameAD(string LoginName)
        {
            try
            {
                PrincipalContext context = new PrincipalContext(ContextType.Domain, ConfigurationManager.AppSettings["ADDomain"], ADUser, ADPassword);

                return UserPrincipal.FindByIdentity(context, LoginName.Trim());
            }
        ...
        public static UserPrincipal isExistsUserNameAD(string LoginName, string Senha)
        {
            try
            {
                PrincipalContext context = new PrincipalContext(ContextType.Domain, ConfigurationManager.AppSettings["ADDomain"], ADUser, ADPassword);
        ...
        {
            DirectoryEntry oDE = new DirectoryEntry(ADPath, UserName, Password, AuthenticationTypes.Secure);
        ...
        public static string GetLDAPDomain()
        {
            ...
            string[] LDAPDC = ConfigurationManager.AppSettings["ADServer"].ToString().Split('.');
        ...
        #region Dados Protegidos
        /// <summary>
        /// Desproteger senha
        /// </summary>
        /// <param name="strConnAtiva">string</param>
        ...
        /// Proteger senha
        /// </summary>
        /// <param name="connStr">string</param>
        /// <returns>string</returns>
        public static string Encrypt(string strSenha)
        {
            try
            {
                Byte[] b = ASCIIEncoding.UTF8.GetBytes(strSenha);
                string strEncrypted = Convert.ToBase64String(b);
                return strEncrypted;
            }
            catch
            {
