                ...
                case "mundipagg_creditcard":
                    {
                        ...
                        paymentModel.DocDate = DateTime.ParseExact(model.updated_at, "yyyy-MM-dd HH:mm:ss", null).AddHours(-3).ToString("yyyy-MM-dd HH:mm:ss");
                        paymentModel.DueDate = DateTime.ParseExact(model.updated_at, "yyyy-MM-dd HH:mm:ss", null).AddHours(-3).ToString("yyyy-MM-dd HH:mm:ss");
                        paymentModel.TaxDate = DateTime.ParseExact(model.updated_at, "yyyy-MM-dd HH:mm:ss", null).AddHours(-3).ToString("yyyy-MM-dd HH:mm:ss");
                        ...
                        if (paymentCreditModel.NumOfPayments > 1)
                        {
                            ...
                            // Primeira parcela recebe a diferença das outras.
                            paymentCreditModel.FirstPaymentSum = (float)(paymentCreditModel.FirstPaymentSum + (u - Math.Round(paymentCreditModel.AdditionalPaymentSum * paymentCreditModel.NumOfPayments, 2)));
                ...
                case "braspag_pagador_creditcard":
                    {
                        ...
                        paymentModel.DocType = "rCustomer";
                        DateTime dateTime = DateTime.Parse(model.updated_at, new CultureInfo("en-US"));
                        paymentModel.DocDate = dateTime.ToString("yyyy-MM-dd");
                        paymentModel.DueDate = dateTime.ToString("yyyy-MM-dd");
                        paymentModel.TaxDate = dateTime.ToString("yyyy-MM-dd");
                        paymentModel.CardCode = dowModel.CardCode;
                        paymentModel.BPLID = dowModel.BPL_IDAssignedToInvoice;
                        paymentModel.U_CVA_EntityId = model.entity_id;
                        paymentModel.U_CVA_Increment_id = Convert.ToInt32(model.increment_id);
                        ...
                        List<JornalEntriesLineModel> journalLines = DAO.FillListFromCommand<JornalEntriesLineModel>(String.Format(HanaCommands.IncomingPayments_GetJournalToFix, DataBase, paymentModel.DocEntry));

                        if (journalLines.Count > 0)
                        {
                            JornalEntriesModel journalModel = new JornalEntriesModel();
                            journalModel.JournalEntryLines = journalLines;
                            string error = await sl.PatchAsync("JournalEntries", journalLines[0].TransId.ToString(), journalModel);
