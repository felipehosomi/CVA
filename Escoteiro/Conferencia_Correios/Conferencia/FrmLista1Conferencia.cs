...
namespace Conferencia
{
    public partial class FrmListaConf1 : Form
    {
        ...
        private void FrmLista1Conferencia_Load(object sender, EventArgs e)
        {
            ...
            var listaConf = _ListaConferenciaBLL.GetListaConf1();
        ...
        private void textBox1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("Nº de pedido de venda não encontrado para a 1ª conferência.");
                    return;
                }

                this.Hide();

                _PedidoBLL.SetAberturaPicking1Conf(Convert.ToInt32(tx_Pedido.Text), Login.ToUpper(), DateTime.Now.ToString());
                _PedidoBLL.InsereStatusEmSeparação(Convert.ToInt32(tx_Pedido.Text));

                var frm = new Form1(Login, tx_Pedido.Text);
                frm.ShowDialog();
            }
        }
    }
