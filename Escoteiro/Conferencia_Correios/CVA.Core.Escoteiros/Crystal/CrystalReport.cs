...
namespace CVA.Core.Escoteiros
{
    public class CrystalReport
    {
        ...
        public void ExecuteCrystalReport(string fileRpt, Hashtable reportParams)
        {
            try
            {
                ...
                //strDSN = Server; //server HANA

                //strAccount = UserID; //usuario

                //strDbName = DataBase; //nombre de la base de datos

                #region OBTENCION DEL PASWORD Y EL NUMERO DE COPIAS DEL REPORTE


                string oPws = Passwrod; //PASWORD PARA ESTA VERSION


                #endregion OBTENCION DEL PASWORD Y EL NUMERO DE COPIAS DE IMPRESION DEL REPORTE

                string strConnection = "DRIVER ={HDBODBC32}; UID =" + UserID; //string de conexion contruido de las variables

                strConnection += ";PWD =" + oPws + "; SERVERNODE =" + Server;

                strConnection += ";CS =" + DataBase;
                ...
                report.DataSourceConnections[0].SetConnection(Server, DataBase, UserID, oPws);
                // 
