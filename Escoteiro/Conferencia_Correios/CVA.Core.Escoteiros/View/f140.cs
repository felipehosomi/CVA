...
namespace CVA.Core.Escoteiros.View
{
    public class f140 : BaseForm
    {
        ...
        public f140(ItemEvent itemEvent)
        {
            this.ItemEventInfo = itemEvent;
        }

        public f140(BusinessObjectInfo businessObjectInfo)
        {
            this.BusinessObjectInfo = businessObjectInfo;
        }

        public f140(ContextMenuInfo contextMenuInfo)
        {
            ...
            if (!ItemEventInfo.BeforeAction)
            {
                ...
                if (ItemEventInfo.EventType == BoEventTypes.et_CLICK && ItemEventInfo.ItemUID == "btn_Inte")
                {
                    ...
                    try
                    {
                        ...
                        DBDataSource ds_OINV = Form.DataSources.DBDataSources.Item("ODLN");
                        ...
                        //string json = Serializar<Shipment_OrderModel>(_Shipment_OrderModel);
                        ...
                        System.IO.File.WriteAllText(local + DateTime.Now.ToString("yyyyMMdd_HHmm_") + _Shipment_OrderModel.order_number + "_Entrega_Enviado.json", json);
                        ...
                        System.IO.File.WriteAllText(local + DateTime.Now.ToString("yyyyMMdd_HHmm_") + _Shipment_OrderModel.order_number + "_Entrega_Retorno.json", response.Content);
                        ...
                        oRecordSet.DoQuery(String.Format(Query.UpadateStatusIntegradoODLN, docentry));
        ...
        private List<IntelipostModel> _IntelipostModel(string DocEntry)
        {
            ...
            SAPbobsCOM.Recordset oRecordSet = (SAPbobsCOM.Recordset)SBOApp.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
            oRecordSet.DoQuery(String.Format(Query.CarregaObjIntelipost_ODLn, DocEntry));
        ...
        private List<IntelipostVolume> _IntelipostVolume(string DocEntry)
        {
            ...
            if (!oRecordSet.EoF)
            {
                for (int i = 0; i < oRecordSet.RecordCount; i++)
                {
                    ...
                    model.weight = Convert.ToInt32(oRecordSet.Fields.Item(3).Value);
            ...
            if (!BusinessObjectInfo.BeforeAction)
            {
                if (BusinessObjectInfo.EventType == BoEventTypes.et_FORM_DATA_ADD)
                {
                    try
                    {
                        ...
                        DBDataSource ds_GC = Form.DataSources.DBDataSources.Item("ODLN");
                        ...
                        if (Integrado == "S" && header == "Entrega - Cancelamento")
                        {
                            ...
                            System.IO.File.WriteAllText(local + DateTime.Now.ToString("yyyyMMdd_HHmm_") + _CancelOrder.order_number + "_Entrega_Cancelamento_Enviado.json", json);
                            ...
                            System.IO.File.WriteAllText(local + DateTime.Now.ToString("yyyyMMdd_HHmm_") + _CancelOrder.order_number + "_Entrega_Cancelamento_Retorno.json", response.Content);
                            ...
                            {
                                SBOApp.Application.SetStatusBarMessage("Cancelamento no Intelipost não realizado, Verifique o arquivo de retorno !!! ", BoMessageTime.bmt_Short, true);
                            }
