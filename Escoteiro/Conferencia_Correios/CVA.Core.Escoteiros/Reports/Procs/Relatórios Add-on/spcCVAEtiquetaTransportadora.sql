CREATE PROCEDURE "spcCVAEtiquetaTransportadora"
(
  layout nvarchar(72)
, Objtype nvarchar(72)
, key nvarchar(72)
...
select top 1 
       ...
       ,(select "CardFName" from OCRD where "CardCode" = T5."Carrier") as "Transportadora"
       ,(select '('||"Phone2"||') ' || "Phone1" from OCRD where "CardCode" = T5."Carrier") as "Fone"
  from ORDR T0
 ...
 where T0."DocNum"  = :key
   ...
   order by 4;
	  ...
	  do
		...
		pedido  := c."DocNum";
		cliente := substring(c."CardName",0,45);
		c_1 := substring(c."CardName",46,45);
 	    endereco_Cliente := substring(c."End_Cliente",0,48);   
 	    end_2 := substring(c."End_Cliente",49,48); 	
 	    end_3 := substring(c."End_Cliente",99,48); 	
 	    end_4 := substring(c."End_Cliente",100,48); 	
 	    end_5 := substring(c."End_Cliente",149,48);     
 	    cod_bar := c."Cep";
		...
		nf := c."Serial";		
		transp := substring(c."Transportadora",0,18);
		trnsp_2 := substring(c."Transportadora",19,18);
		trp_3 := substring(c."Transportadora",38,18);
		fone := c."Fone";
    	...
    	script = replace(:script, '@c_1', ifnull(:c_1, ''));	 
		...
		script = replace(:script, '@transp_1', ifnull(:transp, ''));
		script = replace(:script, '@trnsp_2', ifnull(:trnsp_2, ''));	
		script = replace(:script, '@trp_3', ifnull(:trp_3, ''));
		script = replace(:script, '@fone', ifnull(:fone, ''));					  	
