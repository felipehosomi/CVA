...
using Electra.Currency.BLL;
using Electra.Currency.Extensions;
    ...
    public class CurrencyTaskHelper
    {
        ...
        public void Execute()
        {
            ...
            try
            {
                ...
                string connectionError = ConnectionBLL.ConnectToCompany();
                if (String.IsNullOrEmpty(connectionError))
                {
                
                    List<string> list = CurrencyBLL.GetCurrencies();
                    ...
                    foreach (string line in str.Split(new char[] { '\n' }))
                    {
                        try
                        {
                            ...
                            if ((strArray.Length >= 5) && list.Contains(strArray[3]))
                            {
                                ...
                                sMoeda = strArray[3];
                                sMoeda = CurrencyBLL.GetCurrency(sMoeda);
                                ...
                                CurrencyBLL.UpdateCurrency(sMoeda, Convert.ToDouble(sCurrency, System.Globalization.CultureInfo.GetCultureInfo("pt-BR")), baseDateInit);
                            }
                            else
                            {
                ...
                {
                    LogService.GravarException(String.Format(@"Erro ao conectar na DI API.Detalhes: {0} - {1}", connectionError, sData));
                }
            }
            catch (Exception exception)
            {
