...
namespace CVA.AddOn.Common.Util
{
    public class FormattedSearchUtil
    {
        ...
        public int CreateQuery(string QueryName, string TheQuery, string categoryName = "Geral")
        {
            ...
            try
            {
                string sql = "SELECT TOP 1 INTRNALKEY FROM OUQR WHERE QCATEGORY=" + GetSysCatID(categoryName) + " AND QNAME='" + QueryName + "'";
                oRS.DoQuery(sql);
        ...
        public int GetSysCatID(string name = "Geral")
        {
            ...
            try
            {
                SBOApp.TranslateToHana($"SELECT TOP 1 CATEGORYID FROM OQCN WHERE CATNAME = '{name}'");
                oRS.DoQuery($"SELECT TOP 1 CATEGORYID FROM OQCN WHERE CATNAME = '{name}'");
                if (oRS.RecordCount > 0)
