create view "OPAY_View" as 
	...
	select ORCT."DocEntry" as "PmntEntry", 'ORCT' as "PmntTable", OJDT."RefDate" as "PaymentDate", ORCT."Canceled",
	 ...
	 inner join "OBOT" on OBOT."AbsEntry" = (select "AbsEntry"
 	 										   from (select row_number()over(order by BOT1."AbsEntry" desc) as "RowNumber", BOT1."AbsEntry"
													   from BOT1
