create procedure [dbo].[SP_GetCashFlowInfo]
(
	@fromDate datetime,
	@toDate datetime,
	@partType char
...
begin
	...
	begin
		select ODOC.DocEntry, ODOC.DocNum, ODOC.BPLId, ODOC.TransType, ODOC.ObjType, 
			   ...
			   (select OCRD.U_SubGrupoDespesa from OCRD  where OCRD.CardCode = ODOC.CardCode) as U_SubGrupoDespesa
		  from ODOC_View as ODOC
		 inner join DOC6_View as DOC6 on DOC6.DocEntry = ODOC.DocEntry  
		   and DOC6.ObjType = ODOC.ObjType
		 where cast(convert(char(8), ODOC.UpdateDate, 112) + ' ' + left(right('000000' + cast(ODOC.UpdateTS as varchar(6)), 6), 2) + ':' + SUBSTRING(right('000000' + cast(ODOC.UpdateTS as varchar(6)), 6), 3, 2) + ':' + right(right('000000' + cast(ODOC.UpdateTS as varchar(6)), 6), 2) as datetime) >= @fromDate
		   and cast(convert(char(8), ODOC.UpdateDate, 112) + ' ' + left(right('000000' + cast(ODOC.UpdateTS as varchar(6)), 6), 2) + ':' + SUBSTRING(right('000000' + cast(ODOC.UpdateTS as varchar(6)), 6), 3, 2) + ':' + right(right('000000' + cast(ODOC.UpdateTS as varchar(6)), 6), 2) as datetime) <= @toDate
	end
	-- Abertura Financeira
	else if @partType = 'F' 
	begin
	...
	begin
		select ODOC.DocEntry, ODOC.EventType, JDT1.Line_ID, JDT1.Account, ODOC.CANCELED as JrnlCancel,
				INV1.ItemCode as ItemCode,
			   JDT1.Credit + JDT1.Debit as JrnlLineValue, JDT1.RefDate, JDT1.DueDate,
