...
namespace DAO.Classes
{
    public class ClientDAO
    {
        ...
        public int Add_SalesChannel(ClientModel client, int salesChannelId)
        {
            try
            {
                using (Connection conn = new Connection())
                {
                    conn.CreateDataAdapter(StoredProcedure.Add_SalesChannel);
                    conn.InsertParameter("USR", client.User.Id);
                    conn.InsertParameter("STU", client.Status.Id);
                    conn.InsertParameter("CLI_ID", client.Id);
                    conn.InsertParameter("CAN_VEN_ID", salesChannelId);
