...
namespace BLL.Classes
{
    public class SubPeriodBLL
    {
        ...
        public MessageModel Save(SubPeriodModel model)
        {
            int success = _subPeriodDAO.Save(model);
            return MessageBLL.Generate("Sub-período incluído com sucesso!", success);
        }

        public List<SubPeriodModel> Get(int? colId, int? clientId, int? projectId, DateTime? dateFrom, DateTime? dateTo)
        {
            var result = _subPeriodDAO.Get(colId, clientId, projectId, dateFrom, dateTo);
