cvaGestao.controller('NoteExtractedController', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
    ...
    $scope.Search = function () {
        if (($("#initialDate").attr('class').indexOf('invalid') != -1) || ($("#finishDate").attr('class').indexOf('invalid') != -1))
            ...
            $http({
                method: 'GET',
                url: '/Apontamento/FiltrarInterno?initialDate=' + $filter('date')($scope.note.InitialDate, 'mediumDate') + '&finishDate=' + $filter('date')($scope.note.FinishDate, 'mediumDate') + '&projectId=' + $scope.note.ProjectId + '&clientID=' + $scope.note.ClientId
            }).success(function (result) {
                if (result.length == 0) {
