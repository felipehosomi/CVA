...
namespace BLL.Classes
{
    public class ExpenseBLL
    {
        ...
        public List<ExpenseModel> Get_ByUser(int id, int mes, int ano)
        {
            var result = _expenseDAO.Get_ByUser(id, mes, ano);
        ...
        public MessageModel ValidateModel(ExpenseModel model)
        {
            ...
            var collaboratorModel = _userBLL.GetCollaborator(model.User.Id);

            bool valid = false;
            var diasAutorizados = _authorizationBLL.Get_DespesasAutorizados(collaboratorModel.Id);
            foreach (var item in diasAutorizados)                                                          //
            {                                                                                              //
                if (model.Data.Date >= item.De && model.Data.Date <= item.Ate)                             //
                {                                                                                          //
                    valid = true;                                                                          //
                    break;                                                                                 //
                }                                                                                          //
            ...
            if (!valid)
            {
                if (model.Data.Date.Month != DateTime.Today.Month || model.Data.Date.Year != DateTime.Today.Year)
                {
