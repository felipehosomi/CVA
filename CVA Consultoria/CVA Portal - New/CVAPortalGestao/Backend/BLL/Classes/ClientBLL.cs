...
namespace BLL.Classes
{
    public class ClientBLL
    {
        ...
        public ClientModel LoadModel(DataTable result)
        {
            var model = new ClientModel();

            model.Status = new StatusModel();
            model.Addresses = new List<AddressModel>();
            model.Contact = new List<ContactModel>();
            model.PoliticExpense = new List<PoliticExpenseModel>();

            model.Id = Convert.ToInt32(result.Rows[0]["Id"].ToString());
            model.Name = result.Rows[0]["Name"].ToString();
            model.FantasyName = result.Rows[0]["FantasyName"].ToString();
            model.CNPJ = result.Rows[0]["CNPJ"].ToString();
            model.Description = result.Rows[0]["Description"].ToString();
            model.DescriptionAMS = result.Rows[0]["DescriptionAMS"].ToString();
            model.LocalPoliticExpense = Convert.ToInt32(result.Rows[0]["LocalPoliticExpense"].ToString());
            model.Tag = result.Rows[0]["Tag"].ToString();
            model.Status.Id = Convert.ToInt32(result.Rows[0]["StatusId"].ToString());
        ...
        public MessageModel ValidateModel(ClientModel client)
        {
            ...
            if (string.IsNullOrEmpty(client.Description))
                client.Description = "";
            var dt = _clientDAO.SearchByCNPJ(client.CNPJ);
            if (dt.Rows.Count > 0)
                  return MessageBLL.Generate("Já existe um cliente cadastrado para este CNPJ", 99, true);
