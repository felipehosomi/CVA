cvaGestao.controller('NoteAdministratorController', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
    ...
    $scope.Search = function () {
        if (ValidateFields()) {
            $http({
                method: 'GET',
                url: '/Apontamento/Filtrar?user=' + $scope.Colaborador + '&initialDate=' + $filter('date')($scope.De, 'mediumDate') + '&finishDate=' + $filter('date')($scope.Ate, 'mediumDate') + '&projectId=' + $scope.Projeto + '&clientID=' + $scope.Cliente + '&Chamado=' + $scope.Chamado + '&interno=false'
            }).success(function (result) {
                if (result.length == 0) {
    ...
    function ValidateFields() {
        ...
        if ($scope.Chamado != undefined && $scope.Chamado != 0 && ($scope.Cliente == undefined || $scope.Cliente == 0)) {
