...
namespace CVA.Core.Cianet.BLL
{
    public class RegraFreteBLL
    {
        ...
        public double CalculaFrete(string cliente, string produto, string quantidade, string totalProduto)
        {
            var porcentagem = _dao.CalculaFrete(cliente, produto, quantidade);

            var aux = totalProduto.Replace(".", "");
            totalProduto = aux.Substring(2, totalProduto.Length - 5);
            return ((porcentagem/100) * Convert.ToDouble(totalProduto));
        }
