        ...
        public void SBO_Application_MenuEvent(ref SAPbouiCOM.MenuEvent pVal, out bool BubbleEvent)
        {
            ...
            try
            {
                if (pVal.BeforeAction)
                {
                    try
                    {
                        if (pVal.MenuUID == "PackConfig")
                        {
                            SAPbouiCOM.MenuItem menu = Application.SBO_Application.Menus.Item("51200");

                            for (int i = 0; i < menu.SubMenus.Count; i++)
                            {
                                if (menu.SubMenus.Item(i).String.StartsWith("CVA_PACK_CONFIG"))
                                {
