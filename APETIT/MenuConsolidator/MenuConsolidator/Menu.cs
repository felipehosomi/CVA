        ...
        public void SBO_Application_MenuEvent(ref SAPbouiCOM.MenuEvent pVal, out bool BubbleEvent)
        {
            ...
            try
            {
                if (pVal.BeforeAction)
                {
                    try
                    {
                        if (Application.SBO_Application.Forms.ActiveForm.TypeEx == "MenuConsolidator.Views.BranchTransfer")
                        {
                            Application.SBO_Application.Forms.ActiveForm.Items.Item("mtTransfer").Enabled = false;
                        }

                        var activeForm = Activator.CreateInstance(Type.GetType($"{GetType().Namespace}.Views.{pVal.MenuUID}")) as UserFormBase;
                        activeForm.Show();
                    }
                    catch (Exception ex)
                    {
                ...
                else
                {
                    if (Application.SBO_Application.Forms.ActiveForm.TypeEx == "MenuConsolidator.Views.BranchTransfer")
                    {
                        Application.SBO_Application.Forms.ActiveForm.Items.Item("mtTransfer").Enabled = true;
                    }
                }
            }
            catch (Exception ex)
