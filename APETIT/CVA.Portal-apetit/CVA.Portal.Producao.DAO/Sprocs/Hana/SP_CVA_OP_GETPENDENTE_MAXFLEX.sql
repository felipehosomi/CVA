...
BEGIN
	SELECT DISTINCT
		...
		CASE WHEN UPPER("OITM"."U_CVA_MEDIDAS") = 'SOB MEDIDA' AND "RDR1"."DocEntry" IS NOT NULL
			THEN IFNULL(TO_DECIMAL("RDR1"."U_akblength", 10, 3), '0.00') || ' x ' || IFNULL(TO_DECIMAL("RDR1"."U_akbwidth", 10, 3), '0.00') || ' x ' || IFNULL(TO_DECIMAL("RDR1"."U_akbheight", 10, 3), '0.00') || ' - Dobrável: ' || IFNULL("RDR1"."U_akbfold", 'Não')
			ELSE TO_DECIMAL("OITM"."SLength1", 10, 3) || ' x ' || TO_DECIMAL("OITM"."SWidth1", 10, 3) || ' x ' ||  TO_DECIMAL("OITM"."SHeight1", 10, 3)
		END 				"Medidas",
