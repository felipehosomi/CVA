...
namespace CVA.Portal.Producao.DAO.Util
{
    public class SLConnection
    {
        ...
        private async Task Login(bool forceLogin = false)
        {
            ...
            sessionCookie = new Cookie("B1SESSION", loginResponse.SessionId);
        }
            ...
            uint? top = null, string skip = null, string apply = null, string expand = null)
        {
            ...
            {
                var getRequest = BuildGetRequest(resource, filter, select, orderBy, top, skip, apply, expand);
            ...
            while (true)
            {
                ...
                catch (FlurlHttpException ex)
                {
                    try
                    {
                        ...
                        string responseString = await ex.Call.Response.Content.ReadAsStringAsync();
                    ...
                    if (!returnCodesToRetry.Any(x => x == ex.Call.Response.StatusCode))
                    {
                        break;
                    }
