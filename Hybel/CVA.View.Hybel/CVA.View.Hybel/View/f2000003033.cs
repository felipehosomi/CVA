...
namespace CVA.View.Hybel.View
{
    ...
    /// Montadora
    /// </summary>
    public class f2000003033 : BaseForm
    {
        private Form Form;
        ...
        public f2000003033(SAPbouiCOM.ItemEvent itemEvent)
        {
            this.ItemEventInfo = itemEvent;
        }

        public f2000003033(SAPbouiCOM.BusinessObjectInfo businessObjectInfo)
        {
            this.BusinessObjectInfo = businessObjectInfo;
        }

        public f2000003033(SAPbouiCOM.ContextMenuInfo contextMenuInfo)
        {
        ...
        public override bool FormDataEvent()
        {
            if (BusinessObjectInfo.BeforeAction)
            {
                if (BusinessObjectInfo.EventType == BoEventTypes.et_FORM_DATA_ADD || BusinessObjectInfo.EventType == BoEventTypes.et_FORM_DATA_UPDATE)
                {
                    ...
                    if (String.IsNullOrEmpty(((EditText)mt_Item.GetCellSpecific("cl_Prod", mt_Item.RowCount)).Value))
                    {
        ...
        private void ClItemOnValidate()
        {
            ...
            {
                Matrix mt_Item = (Matrix)Form.Items.Item("mt_Item").Specific;
                EditText cl_Prod = (EditText)mt_Item.GetCellSpecific("cl_Prod", ItemEventInfo.Row);

                if (!String.IsNullOrEmpty(cl_Prod.Value))
                {
        ...
        private void AddRow()
        {
            DBDataSource dt_Fields = Form.DataSources.DBDataSources.Item("@CVA_MONT_PROD");
            dt_Fields.Clear();
