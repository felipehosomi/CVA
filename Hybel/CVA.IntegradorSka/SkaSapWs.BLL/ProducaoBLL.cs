...
using SkaSapWs.BLL.HELPER;
using SkaSapWs.BLL.MODEL;
using System;
...
namespace SkaSapWs.BLL
{
    ...
    {
        public List<SspExportProd> BuscaDadosMes()
        {
        ...
        public void GeraArquivoOP(List<SspExportProd> toImport)
        {
        ...
        public void ReturnData(List<SspExportProd> toImport)
        {
            using (var helper = new SqlHelper("ska"))
            {
                foreach (var item in toImport)
                {
                    ...
                    helper.ExecuteNonQuery($"UPDATE SSPExportProd_Log SET STATUS = 1 WHERE Id = {item.Id}");
                }
            }
        }
