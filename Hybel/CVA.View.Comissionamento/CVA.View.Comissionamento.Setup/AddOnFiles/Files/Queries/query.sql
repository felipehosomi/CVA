IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_PagarComissao') DROP PROCEDURE [dbo].[spc_CVA_PagarComissao]
GO
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_Comissoes') DROP PROCEDURE [dbo].[spc_CVA_Comissoes]
GO
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'FN' AND name = 'fn_CVA_PositionCriteria') DROP FUNCTION [dbo].[fn_CVA_PositionCriteria]
GO
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_RelatorioComissoes') DROP PROCEDURE [dbo].[spc_CVA_RelatorioComissoes]
GO
SET ANSI_NULLS ON
GO
...
INSERT INTO #tbl_docs_faturamento
SELECT DISTINCT
	'F' AS Momento
	...
	, T1.VatSum AS TaxSum_IMPADIC_Linha
	, OA_IMP.TaxSum AS TaxSum_IMPINCL_Linha
	, T0.VatSum AS TaxSum_IMPADIC_Parcela
	, OA_IMP2.TaxSum AS TaxSum_IMPINCL_Parcela
	, T0.Installmnt AS Tot_Parcelas
	, OA_MAX.TotItem AS Tot_Itens
	, NULL AS DocStatus
	, T3.GroupCode AS GroupCode
	, OA_MAX.TotalItens AS TotalItens
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN (OA_TOTAL.LineTotal - OA_IMP.TaxSum)+T1.VatSum ELSE 0 END AS LineTotal_Min_IMPINCL
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_TOTAL.LineTotal ELSE 0 END AS LineTotal_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN OA_TOTAL.LineTotal - OA_IMP.TaxSum ELSE 0 END AS LineTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_TOTAL.LineTotal + T1.VatSum ELSE 0 END AS LineTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN (OA_MAX.TotalItens - OA_IMP2.TaxSum)+T0.VatSum ELSE 0 END AS InsTotal_Min_IMPINCL
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_MAX.TotalItens ELSE 0 END AS InsTotal_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN OA_MAX.TotalItens - OA_IMP2.TaxSum ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_MAX.TotalItens + T0.VatSum ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
FROM OINV T0
	INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry AND T1.TargetType <> N'14' AND ISNULL(T1.FreeChrgBP, 'N') = 'N'
	INNER JOIN INV6 T2 ON T0.DocEntry = T2.DocEntry AND T2.InsTotal <> 0
	INNER JOIN OCRD T3 ON T0.CardCode = T3.CardCode
	INNER JOIN OITM T4 ON T1.ItemCode = T4.ItemCode
	INNER JOIN OITB T5 ON T4.ItmsGrpCod = T5.ItmsGrpCod
	INNER JOIN OSLP T6 ON T0.SlpCode = T6.SlpCode
--	LEFT  JOIN OSLP T7 ON T3.SlpCode = T7.SlpCode
	LEFT  JOIN OPRC T8 ON T1.OcrCode2 = T8.PrcCode
	LEFT  JOIN OPRC T9 ON T1.OcrCode = T9.PrcCode
	LEFT  JOIN OMRC T10 ON T4.FirmCode = T10.FirmCode
	LEFT  JOIN OOND T11 ON T3.IndustryC = T11.IndCode
	LEFT  JOIN INV12 T12 ON T0.DocEntry = T12.DocEntry
	LEFT  JOIN OCNT T13 ON T12.CountyB = T13.AbsId
OUTER APPLY (
	SELECT TotItem = COUNT(INV1.LineNum), TotalItens = SUM(INV1.LineTotal) 
	FROM INV1 WHERE INV1.DocEntry = T0.DocEntry AND INV1.TargetType <> N'14' AND ISNULL(INV1.FreeChrgBP, 'N') = 'N'
) AS OA_MAX
OUTER APPLY (
...
SELECT DISTINCT
	...
	, T3.GroupCode AS GroupCode
	, OA_MAX.TotalItens AS TotalItens
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN (T1.LineTotal - OA_IMP.TaxSum) + T1.VatSum ELSE 0 END AS LineTotal_Min_IMPINCL
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN T1.LineTotal ELSE 0 END AS LineTotal_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN T1.LineTotal - OA_IMP.TaxSum ELSE 0 END AS LineTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN T1.LineTotal + T1.VatSum ELSE 0 END AS LineTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN (OA_MAX.TotalItens - OA_IMP2.TaxSum) + T0.VatSum ELSE 0 END AS InsTotal_Min_IMPINCL
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_MAX.TotalItens ELSE 0 END AS InsTotal_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN OA_MAX.TotalItens - OA_IMP2.TaxSum ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_MAX.TotalItens + T0.VatSum ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
FROM ORDR T0
	INNER JOIN RDR1 T1 ON T0.DocEntry = T1.DocEntry AND T1.TargetType NOT IN (N'14', N'13') AND ISNULL(T1.FreeChrgBP, 'N') = 'N'
	INNER JOIN RDR6 T2 ON T0.DocEntry = T2.DocEntry AND T2.InsTotal <> 0
	INNER JOIN OCRD T3 ON T0.CardCode = T3.CardCode
	INNER JOIN OITM T4 ON T1.ItemCode = T4.ItemCode
	INNER JOIN OITB T5 ON T4.ItmsGrpCod = T5.ItmsGrpCod
	INNER JOIN OSLP T6 ON T0.SlpCode = T6.SlpCode
--	LEFT  JOIN OSLP T7 ON T3.SlpCode = T7.SlpCode
	...
	LEFT  JOIN OMRC T10 ON T4.FirmCode = T10.FirmCode
	LEFT  JOIN OOND T11 ON T3.IndustryC = T11.IndCode
	LEFT  JOIN RDR12 T12 ON T0.DocEntry = T12.DocEntry
	LEFT  JOIN OCNT T13 ON T12.CountyB = T13.AbsId
OUTER APPLY (
	SELECT TotItem = COUNT(RDR1.LineNum), TotalItens = SUM(RDR1.LineTotal) 
	FROM RDR1 WHERE RDR1.DocEntry = T0.DocEntry AND RDR1.TargetType NOT IN (N'14', N'13') AND ISNULL(RDR1.FreeChrgBP, 'N') = 'N'
) AS OA_MAX
OUTER APPLY (
	SELECT TaxSum = ISNULL(SUM(DISTINCT RDR4.TaxSum), 0)
	FROM RDR4
		INNER JOIN OSTA ON RDR4.StaCode = OSTA.Code
		INNER JOIN OSTT ON OSTA.[Type] = OSTT.AbsId
		INNER JOIN ONFT ON OSTT.NfTaxId = ONFT.AbsId
	WHERE RDR4.TaxInPrice = 'Y' AND RDR4.DocEntry = T2.DocEntry ANd RDR4.LineNum = T1.LineNum
) AS OA_IMP
OUTER APPLY (
	SELECT TaxSum = ISNULL(SUM(DISTINCT RDR4.TaxSum), 0)
	FROM RDR4
		...
		INNER JOIN OSTT ON OSTA.[Type] = OSTT.AbsId
		INNER JOIN ONFT ON OSTT.NfTaxId = ONFT.AbsId
	WHERE RDR4.TaxInPrice = 'Y' AND RDR4.DocEntry = T2.DocEntry
) AS OA_IMP2
WHERE T0.CANCELED = 'N'
...
SELECT DISTINCT
	...
	, T0.DocTotal AS DocTotal
	, T0.ObjType AS ObjType
	, T16.ItemCode AS ItemCode
	, T16.LineNum AS LineNum
	, T16.Quantity AS Quantity
	, (T14.DrawnSum * T16.LineTotal)/OA_MAX.TotalItens AS LineTotal
	, T2.InstlmntID
	, T2.InsTotal
	, T2.DueDate
	, T4.ItemName AS ItemName
	...
	, T3.GroupCode AS GroupCode
	, OA_MAX.TotalItens AS TotalItens
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN (((T14.DrawnSum * T16.LineTotal)/OA_MAX.TotalItens) - OA_IMP.TaxSum) + T1.VatSum ELSE 0 END AS LineTotal_Min_IMPINCL
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN ((T14.DrawnSum * T16.LineTotal)/OA_MAX.TotalItens) ELSE 0 END AS LineTotal_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN ((T14.DrawnSum * T16.LineTotal)/OA_MAX.TotalItens) - OA_IMP.TaxSum ELSE 0 END AS LineTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN ((T14.DrawnSum * T16.LineTotal)/OA_MAX.TotalItens) + T1.VatSum ELSE 0 END AS LineTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN (OA_MAX.TotalItens - OA_IMP2.TaxSum) + T0.VatSum ELSE 0 END AS InsTotal_Min_IMPINCL
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_MAX.TotalItens ELSE 0 END AS InsTotal_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'N' THEN OA_MAX.TotalItens - OA_IMP2.TaxSum ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_MAX.TotalItens + T0.VatSum ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
FROM ODPI T0
	...
	INNER JOIN DPI6 T2 ON T0.DocEntry = T2.DocEntry AND T2.InsTotal <> 0
	INNER JOIN OCRD T3 ON T0.CardCode = T3.CardCode	
	INNER JOIN INV9 T14 ON T0.DocEntry = T14.BaseAbs AND T0.ObjType = T14.ObjType
	INNER JOIN OINV T15 ON T14.DocEntry = T15.DocEntry
	INNER JOIN INV1 T16 ON T15.DocEntry = T16.DocEntry
	INNER JOIN OITM T4 ON T16.ItemCode = T4.ItemCode
	INNER JOIN OITB T5 ON T4.ItmsGrpCod = T5.ItmsGrpCod
	LEFT  JOIN OSLP T6 ON T15.SlpCode = T6.SlpCode
--	LEFT  JOIN OSLP T7 ON T3.SlpCode = T7.SlpCode
	LEFT  JOIN OPRC T8 ON T16.OcrCode2 = T8.PrcCode
	LEFT  JOIN OPRC T9 ON T16.OcrCode = T9.PrcCode
	LEFT  JOIN OMRC T10 ON T4.FirmCode = T10.FirmCode
	LEFT  JOIN OOND T11 ON T3.IndustryC = T11.IndCode
	LEFT  JOIN INV12 T12 ON T15.DocEntry = T12.DocEntry
	LEFT  JOIN OCNT T13 ON T12.CountyB = T13.AbsId
OUTER APPLY (
	SELECT TotItem = COUNT(INV1.LineNum), TotalItens = SUM(INV1.LineTotal) 
	FROM INV1 WHERE INV1.DocEntry = T16.DocEntry
) AS OA_MAX
OUTER APPLY (
	SELECT TaxSum = ISNULL(SUM(DISTINCT INV4.TaxSum), 0)
	FROM INV4
		...
		INNER JOIN OSTT ON OSTA.[Type] = OSTT.AbsId
		INNER JOIN ONFT ON OSTT.NfTaxId = ONFT.AbsId
	WHERE INV4.TaxInPrice = 'Y' AND INV4.DocEntry = T15.DocEntry ANd INV4.LineNum = T16.LineNum
) AS OA_IMP
OUTER APPLY (
	SELECT TaxSum = ISNULL(SUM(DISTINCT INV4.TaxSum), 0)
	FROM INV4
		...
		INNER JOIN OSTT ON OSTA.[Type] = OSTT.AbsId
		INNER JOIN ONFT ON OSTT.NfTaxId = ONFT.AbsId
	WHERE INV4.TaxInPrice = 'Y' AND INV4.DocEntry = T15.DocEntry
) AS OA_IMP2
WHERE T0.CANCELED = 'N'
...
FROM ORIN T0
	...
	INNER JOIN RIN6 T2 ON T0.DocEntry = T2.DocEntry AND T2.InsTotal <> 0
	INNER JOIN OCRD T3 ON T0.CardCode = T3.CardCode
	INNER JOIN OITM T4 ON T1.ItemCode = T4.ItemCode
	INNER JOIN OITB T5 ON T4.ItmsGrpCod = T5.ItmsGrpCod
	INNER JOIN OSLP T6 ON T0.SlpCode = T6.SlpCode
--	LEFT  JOIN OSLP T7 ON T3.SlpCode = T7.SlpCode
	...
	LEFT  JOIN OMRC T10 ON T4.FirmCode = T10.FirmCode
	LEFT  JOIN OOND T11 ON T3.IndustryC = T11.IndCode
	LEFT  JOIN RIN12 T12 ON T0.DocEntry = T12.DocEntry
	LEFT  JOIN OCNT T13 ON T12.CountyB = T13.AbsId
OUTER APPLY (
	SELECT TotItem = COUNT(RIN1.LineNum), TotalItens = SUM(RIN1.LineTotal) 
	FROM RIN1 WHERE RIN1.DocEntry = T0.DocEntry AND RIN1.TargetType <> N'14' AND ISNULL(RIN1.FreeChrgBP, 'N') = 'N'
) AS OA_MAX
OUTER APPLY (
...
--	LEFT  JOIN OSLP T7 ON T3.SlpCode = T7.SlpCode
	...
	LEFT  JOIN INV12 T12 ON T0.DocEntry = T12.DocEntry
	LEFT  JOIN OCNT T13 ON T12.CountyB = T13.AbsId
	INNER JOIN RCT2 T14 ON T2.DocEntry = T14.DocEntry AND T2.InstlmntID = T14.InstId AND T2.ObjType = T14.InvType
	INNER JOIN ORCT T15 ON T14.DocNum = T15.DocEntry
	LEFT  JOIN OBOE T16 ON T15.BoeAbs = T16.BoeKey
	LEFT  JOIN OBOT T17 ON T17.AbsEntry = 
	(
	...
	LEFT  JOIN OJDT T18 ON T17.TransId = T18.Number
	LEFT  JOIN RCT1 T19 ON T15.DocEntry = T19.DocNum
	LEFT  JOIN OCHH T20 ON T19.CheckAbs = T20.CheckKey
	LEFT  JOIN DPS1 T21 ON T20.CheckKey = T21.CheckKey AND T21.DepCancel = 'N' AND T20.DpstAbs = T21.DepositId
	LEFT  JOIN ODPS T22 ON T21.DepositId = T22.DeposId AND T22.DeposType = 'K'
OUTER APPLY (
	SELECT TotItem = COUNT(INV1.LineNum), TotalItens = SUM(INV1.LineTotal) 
	FROM INV1 WHERE INV1.DocEntry = T0.DocEntry AND INV1.TargetType <> N'14' AND ISNULL(INV1.FreeChrgBP, 'N') = 'N'
) AS OA_MAX
OUTER APPLY (
	SELECT Countt = COUNT(RCT2.InvoiceId) FROM RCT2 WHERE RCT2.DocNum = T15.DocEntry
) AS OA_MAXX
OUTER APPLY (
...
WHERE T0.CANCELED = 'N' AND OA_DATA.DataPagamento IS NOT NULL AND OA_DATA.DataPagamento >= @DataInicial AND OA_DATA.DataPagamento <= @DataFinal
AND (
	SELECT COUNT(OACT.AcctCode) FROM JDT1
	INNER JOIN OACT ON JDT1.Account = OACT.AcctCode AND OACT.Finanse = 'Y'
...
SELECT DISTINCT
	...
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN ((T23.DrawnSum * T25.LineTotal)/OA_MAX.TotalItens) * (T2.InstPrcnt/100) ELSE 0 END AS LineTotal_Min_IMPADIC
...
FROM ODPI T0
	...
	LEFT  JOIN DPS1 T21 ON T20.CheckKey = T21.CheckKey AND T21.DepCancel = 'N' AND T20.DpstAbs = T21.DepositId
	LEFT  JOIN ODPS T22 ON T21.DepositId = T22.DeposId AND T22.DeposType = 'K'
	INNER JOIN INV9 T23 ON T23.BaseAbs = T0.DocEntry AND T23.ObjType = T0.ObjType
	INNER JOIN OINV T24 ON T24.DocEntry = T23.DocEntry
	INNER JOIN INV1 T25 ON T24.DocEntry = T25.DocEntry
	INNER JOIN OITM T4 ON T25.ItemCode = T4.ItemCode
	INNER JOIN OITB T5 ON T4.ItmsGrpCod = T5.ItmsGrpCod
	LEFT  JOIN OSLP T6 ON T24.SlpCode = T6.SlpCode
--	LEFT  JOIN OSLP T7 ON T3.SlpCode = T7.SlpCode
	LEFT  JOIN OPRC T8 ON T25.OcrCode2 = T8.PrcCode
	LEFT  JOIN OPRC T9 ON T25.OcrCode = T9.PrcCode
	LEFT  JOIN OMRC T10 ON T4.FirmCode = T10.FirmCode
	LEFT  JOIN OOND T11 ON T3.IndustryC = T11.IndCode
	LEFT  JOIN INV12 T12 ON T24.DocEntry = T12.DocEntry
	LEFT  JOIN OCNT T13 ON T12.CountyB = T13.AbsId
OUTER APPLY (
	SELECT TotItem = COUNT(INV1.LineNum), TotalItens = SUM(INV1.LineTotal) 
	FROM INV1 WHERE INV1.DocEntry = T24.DocEntry
) AS OA_MAX
OUTER APPLY (
	SELECT Countt = COUNT(RCT2.InvoiceId) FROM RCT2 WHERE RCT2.DocNum = T15.DocEntry
) AS OA_MAXX
...
SELECT DISTINCT
	...
	, -CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'Y' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN T14.SumApplied ELSE 0 END AS InsTotal_Min_IMPINCL_Min_IMPADIC
FROM ORIN T0
	INNER JOIN RIN1 T1 ON T0.DocEntry = T1.DocEntry AND T1.TargetType <> N'14' AND ISNULL(T1.FreeChrgBP, 'N') = 'N'
	INNER JOIN RIN6 T2 ON T0.DocEntry = T2.DocEntry AND T2.InsTotal <> 0
	INNER JOIN OCRD T3 ON T0.CardCode = T3.CardCode
	INNER JOIN OITM T4 ON T1.ItemCode = T4.ItemCode
	INNER JOIN OITB T5 ON T4.ItmsGrpCod = T5.ItmsGrpCod
...
LEFT JOIN #tbl_docs_recebimento T1 ON T0.DocEntry = T1.DocEntry AND T0.ObjType = T1.ObjType AND T0.InstlmntID = T1.InstlmntID
...
OPEN @cur
FETCH NEXT FROM @cur INTO @CritCode, @CritName, @CritPos, @CritAtivo, @RegrCode, @RegrName, @Tipo, @Comissionado, @Momento, @Vendedor, @Item, @Grupo, @CentroCusto, @Fabricante, @Cliente, @Cidade, @Estado, @Setor, @Comissao, @GroupCode
...
insert into #tmp_table_ret
	select 
		...
		CASE WHEN T0.U_OBJTYPE = N'13' THEN
			(SELECT Serial FROM OINV WHERE DocEntry = T0.U_DOCENTRY)
...
--	where T0.U_PAGO = 'N'
--		and (
--			(T0.U_DOCDATE >= @DataInicial and T0.U_DOCDATE <= @DataFinal AND T1.U_MOMENTO <> 'R') 
--			OR (T0.U_TAXDATE >= @DataInicial and T0.U_TAXDATE <= @DataFinal AND T1.U_MOMENTO = 'R')
--		)
--		and @Status IN ('N', 'T')
...
INSERT INTO #tmp_table_ret
	...
	begin
		...
		group by U_COMISSIONADO, 
			U_PRIORIDADE, U_CARDCODE, U_CARDNAME, U_REGRA, U_DOCDATE, U_DUEDATE, U_DOCENTRY, U_OBJTYPE, U_PARCELA, CASE U_MOMENTO WHEN 'R' THEN 'Recebimento' WHEN 'P' THEN 'Pedido' WHEN 'F' THEN 'Faturamento' END, U_SERIAL, ISNULL(U_TAXDATE, '1900-01-01')
	end
	else if @View = 'I'
	begin
		select distinct
...
begin
	...
	if @detalhado = 'Y'
		select 
			...
			T0.U_COMISSAO,
			T0.U_VALORCOMISSAO,
			CASE T1.U_MOMENTO WHEN 'P' THEN 'Pedido' WHEN 'F' THEN 'Faturamento' WHEN 'R' THEN 'Recebimento' END AS U_MOMENTO
		from [@CVA_CALC_COMISSAO] T0
			inner join [@CVA_REGR_COMISSAO] T1 on T0.U_REGRA = T1.Code AND T1.U_ATIVO = 'Y'
		where T0.U_PAGO = 'N'
			and T0.U_DOCDATE >= @dataInicial 
			and T0.U_DOCDATE <= @dataFinal
			and (T0.U_COMISSIONADO = @vendedor or @vendedor = 0)
	else
		select
			T0.U_COMISSIONADO,
			T1.SlpName,
...
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION fn_CVA_PositionCriteria(
	@Vendedor INT = 0
	, @GrupoItens INT = 0
	, @Cliente NVARCHAR(30) = ''
	, @Item NVARCHAR(40) = ''
...
AS
BEGIN
	DECLARE @ret INT, @pos INT, @pos_aux INT
	SELECT @ret = 0, @pos = 99, @pos_aux = 99

	IF @GrupoCliente <> 0
	BEGIN
		SELECT @pos = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '10' AND U_ATIVO = 'Y'
		SELECT @ret = 10
	END
	
	IF @Setor <> 0
	BEGIN
		SELECT @pos = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '9' AND U_ATIVO = 'Y'
		SELECT @ret = 9
	END

	IF @Estado <> ''
	BEGIN
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '8' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 8, @pos = @pos_aux
	END

	IF @Cidade <> 0
	BEGIN
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '7' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 7, @pos = @pos_aux
	END

	IF @Cliente <> ''
	BEGIN
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '6' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 6, @pos = @pos_aux
	END

	IF @Fabricante <> 0
	BEGIN
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '5' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 5, @pos = @pos_aux
	END

	IF @CentroCusto <> ''
	BEGIN
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '4' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 4, @pos = @pos_aux
	END

	IF @GrupoItens <> 0
	BEGIN
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '3' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 3, @pos = @pos_aux
	END

	IF @Item <> ''
	BEGIn
		SELECT @pos_aux = ISNULL(U_POS, 99) FROM [@CVA_CRIT_COMISSAO] WHERE Code = '2' AND U_ATIVO = 'Y'

		IF @pos_aux < @pos
			SELECT @ret = 2, @pos = @pos_aux
	END
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spc_CVA_RelatorioComissoes](@dt_inicial DATETIME, @dt_final DATETIME, @tipo CHAR(1) = 'R')
AS
BEGIN
	SET DATEFORMAT 'ymd';
	SET NOCOUNT ON;
	...
	(
		[CodigoComissionado] INT
		, [NomeComissionado] NVARCHAR(310)
		, [CodigoCliente] NVARCHAR(30)
		, [NomeCliente] NVARCHAR(200)
		, [NumeroDocumento] INT
		, [ParcelaDocumento] INT
		, [ValorBase] NUMERIC(19,6)
		, [ValorImpostos] NUMERIC(19,6)
		, [PercentComissao] NUMERIC(19,6)
		, [ValorComissao] NUMERIC(19,6)
		, [DataPagamentoComissao] DATETIME
		, [DataPagamentoDocumento] DATETIME
		, [DataEmissaoDocumento] DATETIME
		, [DataVencimentoDocumento] DATETIME
		, [NomeItem] NVARCHAR(200)
		, [LinhaDoProduto] NVARCHAR(60)
		, [MomentoComissao] NVARCHAR(11)
	)
	...
	BEGIN
		...
		SELECT 
			T1.SlpCode AS [CodigoComissionado]
			, T1.SlpName AS [NomeComissionado]
			, T0.U_CARDCODE AS [CodigoCliente]
			, T0.U_CARDNAME AS [NomeCliente]
			, ISNULL(OA_INV.Serial, T0.U_DOCENTRY) AS [NumeroDocumento]
			, T0.U_PARCELA AS [ParcelaDocumento]
			, SUM(T0.U_VALOR) AS [ValorBase]
			, SUM(T0.U_IMPOSTOS) AS [ValorImpostos]
			, AVG(T0.U_COMISSAO) AS [PercentComissao]
			, SUM(T0.U_VALORCOMISSAO) AS [ValorComissao]
			, CASE WHEN T0.U_DATAPAGAMENTO = '1900-01-01 00:00:00.000' THEN NULL ELSE T0.U_DATAPAGAMENTO END AS [DataPagamentoComissao]
			, CASE WHEN T0.U_TAXDATE = '1900-01-01 00:00:00.000' THEN NULL ELSE T0.U_TAXDATE END AS [DataPagamentoDocumento]
			, T0.U_DOCDATE AS [DataEmissaoDocumento]
			, T0.U_DUEDATE AS [DataVencimentoDocumento]
			, NULL AS [NomeItem]
			, ISNULL(T2.PrcName, '--') AS [LinhaDoProduto]
			, CASE T3.U_MOMENTO WHEN 'F' THEN 'Faturamento' WHEN 'R' THEN 'Recebimento' WHEN 'P' THEN 'Pedido' END AS [MomentoComissao]
		FROM [@CVA_CALC_COMISSAO] T0
			INNER JOIN OSLP T1 ON T0.U_COMISSIONADO = T1.SlpCode
			LEFT  JOIN OPRC T2 ON T0.U_CENTROCUSTO = T2.PrcCode
			INNER JOIN [@CVA_REGR_COMISSAO] T3 ON T0.U_REGRA = T3.Code
			OUTER APPLY (
				SELECT Serial = P.Serial FROM (
					SELECT OINV.Serial
					FROM OINV WHERE OINV.DocEntry = T0.U_DOCENTRY AND OINV.ObjType = T0.U_OBJTYPE
					UNION ALL
					SELECT ORIN.Serial
					FROM ORIN WHERE ORIN.DocEntry = T0.U_DOCENTRY AND ORIN.ObjType = T0.U_OBJTYPE
				) AS P
			) AS OA_INV
		WHERE (((T0.U_DOCDATE >= @dt_inicial AND T0.U_DOCDATE <= @dt_final) AND T3.U_MOMENTO <> 'R') 
			OR ((ISNULL(T0.U_TAXDATE, '1900-01-01') >= @dt_inicial AND ISNULL(T0.U_TAXDATE, '1900-01-01') <= @dt_final) AND T3.U_MOMENTO = 'R')
			) 
		GROUP BY 
			T1.SlpCode
			, T1.SlpName
			, T0.U_CARDCODE
			, T0.U_CARDNAME
			, ISNULL(OA_INV.Serial, T0.U_DOCENTRY)
			, T0.U_PARCELA
			, CASE WHEN T0.U_DATAPAGAMENTO = '1900-01-01 00:00:00.000' THEN NULL ELSE T0.U_DATAPAGAMENTO END
			, CASE WHEN T0.U_TAXDATE = '1900-01-01 00:00:00.000' THEN NULL ELSE T0.U_TAXDATE END
			, T0.U_DOCDATE
			, T0.U_DUEDATE
			, ISNULL(T2.PrcName, '--')
			, CASE T3.U_MOMENTO WHEN 'F' THEN 'Faturamento' WHEN 'R' THEN 'Recebimento' WHEN 'P' THEN 'Pedido' END
		ORDER BY 1, 16, 4
	END
	ELSE IF @tipo = 'D'
	BEGIN
		...
		SELECT 
			...
			, ISNULL(OA_INV.Serial, T0.U_DOCENTRY) AS [NumeroDocumento]
			, T0.U_PARCELA AS [ParcelaDocumento]
			, T0.U_VALOR AS [ValorBase]
			, T0.U_IMPOSTOS AS [ValorImpostos]
			, T0.U_COMISSAO AS [PercentComissao]
			, T0.U_VALORCOMISSAO AS [ValorComissao]
			, CASE WHEN T0.U_DATAPAGAMENTO = '1900-01-01 00:00:00.000' THEN NULL ELSE T0.U_DATAPAGAMENTO END AS [DataPagamentoComissao]
			, CASE WHEN T0.U_TAXDATE = '1900-01-01 00:00:00.000' THEN NULL ELSE T0.U_TAXDATE END AS [DataPagamentoDocumento]
			, T0.U_DOCDATE AS [DataEmissaoDocumento]
			, T0.U_DUEDATE AS [DataVencimentoDocumento]
			, T0.U_ITEMNAME AS [NomeItem]
			, ISNULL(T2.PrcName, '--') AS [LinhaDoProduto]
			, CASE T3.U_MOMENTO WHEN 'F' THEN 'Faturamento' WHEN 'R' THEN 'Recebimento' WHEN 'P' THEN 'Pedido' END AS [MomentoComissao]
		FROM [@CVA_CALC_COMISSAO] T0
			INNER JOIN OSLP T1 ON T0.U_COMISSIONADO = T1.SlpCode
	...
	SELECT * FROM #tmp_return ORDER BY 1, 16, 4
