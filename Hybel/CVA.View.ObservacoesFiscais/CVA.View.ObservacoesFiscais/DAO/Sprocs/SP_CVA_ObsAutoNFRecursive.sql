-- declare @a varchar(max); exec SP_CVA_ObsAutoNFRecursive 'oinv', '', @a output; select @a
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'SP_CVA_ObsAutoNFRecursive')
	DROP PROCEDURE SP_CVA_ObsAutoNFRecursive
GO
CREATE PROC SP_CVA_ObsAutoNFRecursive
...
select @sql = '
  insert into #cvaobsaut1 (code, codepai, texto, lineid, itemcode, usage, taxcode, cfop, ncm, cardcode, groupcode, state, city, bplid, sql, itmsgrpnam)
    select a.Code, b.Code, b.U_texto, a.LineId, a.U_itemcode, a.U_usage, a.U_taxcode, a.U_cfop, a.U_ncm, a.U_cardcode, a.U_groupnum, a.U_state, a.U_city, a.U_bplid, a.U_sql, a.U_itmsgrpnam
      from [@CVA_OBSAUTO] b
      left join [@CVA_OBSAUTO1] a
        on a.code = b.code
       and isnull(a.U_status, ''Y'') = ''Y''
     where b.U_status = ''Y''
    ...
    begin
      ...
      if exists (select 0
                  ...
                  where U_parent = @vcodepai
                    and isnull(U_parent, '') <> '')
        exec SP_CVA_ObsAutoNFRecursive @tablename, @vcodepai, @texto output
