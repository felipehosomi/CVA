...
namespace CVA.View.DIME.VIEW
{
    ...
    /// Quadro 46
    /// </summary>
    public class f2000005012 : BaseForm
    {
        private Form Form;
        ...
        public f2000005012(SAPbouiCOM.ItemEvent itemEvent)
        {
            this.ItemEventInfo = itemEvent;
        }

        public f2000005012(SAPbouiCOM.BusinessObjectInfo businessObjectInfo)
        {
            this.BusinessObjectInfo = businessObjectInfo;
        }

        public f2000005012(SAPbouiCOM.ContextMenuInfo contextMenuInfo)
        {
            ...
            if (!ItemEventInfo.BeforeAction)
            {
                ...
                if (ItemEventInfo.EventType == SAPbouiCOM.BoEventTypes.et_LOST_FOCUS)
                {
                    ...
                    if (ItemEventInfo.ItemUID == "mt_Item")
                    {
                        ...
                        {
                            if (!String.IsNullOrEmpty(((EditText)mt_Item.GetCellSpecific("cl_Ident", ItemEventInfo.Row)).Value))
                            {
                                this.AddRow();
                            }
                        }
        ...
        public override bool FormDataEvent()
        {
            if (BusinessObjectInfo.BeforeAction)
            {
                if (BusinessObjectInfo.EventType == BoEventTypes.et_FORM_DATA_ADD)
                {
                    ...
                    DBDataSource dt_DIME = Form.DataSources.DBDataSources.Item("@CVA_DIME_46");
                    ...
                    {
                        if (CrudController.ExecuteScalar(String.Format(SQL.DimeQuadro46_Get, period, filial)) != null)
                        {
                    ...
                    dt_DIME.SetValue("Code", dt_DIME.Offset, CrudController.GetNextCode("@CVA_DIME_46").PadLeft(4, '0'));
                }
            }
        ...
        private void AddRow()
        {
            DBDataSource dt_Item = Form.DataSources.DBDataSources.Item("@CVA_DIME_46_ITEM");
            dt_Item.Clear();
