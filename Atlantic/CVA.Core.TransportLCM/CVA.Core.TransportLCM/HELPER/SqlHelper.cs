...
namespace CVA.Core.TransportLCM.HELPER
{
    class SqlHelper
    {
        ...
        {
            List<T> list = FillModelList<T>(sql);
            if (list.Count > 0)
            {
                return list[0];
            }
        ...
        public List<T> FillModelList<T>(string sql)
        {
            ...
            ModelHelperAttribute modelHelper;
                    ...
                    while (dr.Read())
                    {
                        ...
                        foreach (PropertyInfo property in model.GetType().GetProperties())
                        {
                            try
                            {
                                ...
                                {
                                    modelHelper = attribute as ModelHelperAttribute;
                                    if (modelHelper != null)
                                    {
                                        if (String.IsNullOrEmpty(modelHelper.ColumnName))
                                        {
                                            modelHelper.ColumnName = property.Name;
                                        }

                                        int index = dr.GetOrdinal(modelHelper.ColumnName);
        ...
        /// </summary>
        public void CreateModel<T>(string tableName, T model)
        {
            try
            {
                    ...
                    try
                    {
                        foreach (Attribute attribute in property.GetCustomAttributes(true))
                        {
                            ...
                            if (modelHelper != null)
                            {
                                ...
                                sqlColumns += String.Format(", {0}", modelHelper.ColumnName);
                ...
                {
                    throw new Exception("Nenhuma coluna informada. Informe as propriedades no Model");
                }
