...
namespace CVA.Core.TransportLCM.HELPER
{
    public class FormattedSearch
    {
        ...
        public FormattedSearch(Company company)
        {
            _company = company;
        }
        ...
        public bool AssignFormattedSearch(string QueryName, string TheQuery, string FormID, string ItemID, string ColID = "-1")
        {
            ...
            SAPbobsCOM.Recordset oRS = (Recordset)_company.GetBusinessObject(BoObjectTypes.BoRecordset);
            SAPbobsCOM.FormattedSearches oFS = (FormattedSearches)_company.GetBusinessObject(BoObjectTypes.oFormattedSearches);
            ...
            try
            {
                ...
                if (oRS.RecordCount == 0)
                {
                    ...
                    if (lRetCode != 0)
                    {
                        throw new Exception(String.Format("Erro ao criar query {0}: {1}", QueryName, _company.GetLastErrorDescription()));
                    }
                }
                else
                {
        ...
        public void RemoveFormattedSearch(string queryName, string itemId, string formId)
        {
            ...
            SAPbobsCOM.UserQueries oQuery = (UserQueries)_company.GetBusinessObject(BoObjectTypes.oUserQueries);
            SAPbobsCOM.FormattedSearches oFS = (FormattedSearches)_company.GetBusinessObject(BoObjectTypes.oFormattedSearches);
        ...
        public int CreateQuery(string QueryName, string TheQuery)
        {
            ...
            try
            {
                ...
                if (oRS.RecordCount > 0)
                {
                    ...
                    oQuery.GetByKey(functionReturnValue, GetSysCatID());
        ...
        public int GetSysCatID()
        {
            ...
            {
                throw new Exception(String.Format("Erro: {0}", _company.GetLastErrorDescription()));
            }
            finally
            {
                System.Runtime.InteropServices.Marshal.ReleaseComObject(oRS);
