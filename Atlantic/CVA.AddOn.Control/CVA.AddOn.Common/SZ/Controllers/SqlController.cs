...
namespace SZS.Common.Controllers
{
    public class SqlController
    {
        ...
        {
            this.ConnectionString = this.GetConnectionString(SZApp.ServerName, SZApp.DatabaseName, SZApp.DBUserName, SZApp.DBPassword);
        }
        ...
        public void CreateModel()
        {
            try
            {
                ...
                try
                {
                    docEntry = Convert.ToInt32(this.ExecuteScalar(String.Format("SELECT ISNULL(MAX(DocEntry), 0) + 1 FROM [{0}]", TableName)));
                }
                catch
                {
                    // Se deu erro é porque não existe a coluna DocEntry
                }

                if (docEntry > 0)
                {
                    typesList.Add(docEntry.GetType());
                    valuesList.Add(docEntry);
                    sqlColumns += ", DocEntry";
                }
