...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spcCVAPedidoDeComprasContaContabilSaldo]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].spcCVAPedidoDeComprasContaContabilSaldo
GO
...
CREATE PROCEDURE spcCVAPedidoDeComprasContaContabilSaldo
	@DocNum int
...
BEGIN
	...
	where
		...
		--	)
		--and (@ContaContabil='*' or @ContaContabil=[CONTA CONTÁBIL])
end

go

execute spcCVAPedidoDeComprasContaContabilSaldo 0,'*','*','*','*','*','*' ,'*','2018-01-01','2018-12-30'
	--@CArdCode as nvarchar(max)
	--,@CArdRazaoSocial as nvarchar(max)
	--,@CArdNomeFantasia as nvarchar(max)		
	--,@CNPJ as nvarchar(max)
