...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spcCVAComposicaoSaldoContabilTitulos]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].spcCVAComposicaoSaldoContabilTitulos
GO
...
CREATE PROCEDURE spcCVAComposicaoSaldoContabilTitulos
	@DataIni as DAte
	,@DataFIM as DAte
...
--203 A/R Down Payment 
	...
	where
		...
		and (@CNPJ='*' or exists ( select 1 from  CRD7 T6 where  T6.CardCode=T0.CardCode and (not T6.TaxId0 is null or not T6.TaxId4 is null) and coalesce(T6.TaxId0,T6.TaxId4)=@CNPJ  )    )
		and (@BplId=0 or @BplId=T0.BPLId)
		--AND (@TIPOCONTA='*' OR @TIPOCONTA='PAGAR' )
	UNION ALL
--select top 1 ObjType  from OINV		
--13 A/R Invoice 
	select 
...
--select top 1 ObjType  from ORIN
--14 A/R Credit Memo 	
--SELECT * FROM VPM2 WHERE invType=14
	UNION ALL
	select 
		'PAGAR' as 'TPMov' 
		,T0.DocEntry
...
execute spcCVAComposicaoSaldoContabilTitulos '2018-01-01','2020-12-30','*','*','*','*',0,'*'
