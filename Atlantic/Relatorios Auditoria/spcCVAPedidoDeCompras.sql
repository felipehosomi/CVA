...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spcCVAPedidoDeCompras]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].spcCVAPedidoDeCompras
GO
...
CREATE PROCEDURE spcCVAPedidoDeCompras
	@DocNum int
...
BEGIN
	select * from (
		select distinct
			  ...
			  else 'Não encaixa na regra da especificação' end) as 'SituacaoEntrega'
	...
	where
		(upper(@SituacaoEntrega)='*' or upper(@SituacaoEntrega)=TB.SituacaoEntrega)
		and (
				(  @AprovadorIni='Todos' and @AprovadorFim='Todos' )
				or (  TB.Aprovador>=upper(@AprovadorIni) and TB.Aprovador<=upper(@AprovadorFim))
			)
end

go

execute spcCVAPedidoDeCompras 0,'*','*','*','*','*',0 ,'2018-01-01','2018-12-30','Todos','Todos','Todos','Todos','*'
