...
namespace CVA_Rep_DAL
{
    public class AcessoDAO
    {
        ...
        public bool Grant_Access(string senha, string email)
        {
            var hasAccess = false;
            ...
            if (oConnection != null)
            {
                if (oConnection.State == System.Data.ConnectionState.Open)
                {
                    var cmd = new SqlCommand($"SELECT SENHA FROM [CVA_ATL_PORTAL].[DBO].CVA_USER WHERE EMAIL = '{email}'", oConnection);
                    var reader = cmd.ExecuteReader();

                    while (reader.Read())
                    {
                        var senhaUsuario = EncryptBlo.Decrypt(reader["SENHA"].ToString(), email); 
