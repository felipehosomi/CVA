...
aLTER proc spcCVAEtiquetaLogisticaComBorda
...
Begin
 ...
 declare @endereco_3		varchar(250)
 declare @horario		    varchar(250)
 declare @horario_1		    varchar(250)
 declare @horario_2		    varchar(250)
 ...
 select @campolivre1,@campolivre2
  ...
  select @script  = U_script
   ...
   --select @script
	...
	create table #Hora (hora varchar(50) null, ordem int not null identity)
...
----------------------------------------------------------------------------------------
	...
	select @Cliente_2 = cliente from #cliente where ordem = 6
	select @Cliente_2 = @Cliente_2 + ' ' + cliente from #cliente where ordem = 7
...
----------------------------------------------------------------------------------------
	...
	select @endereco_2 = endereco from #endereco where ordem = 3
	select @endereco_2 = @endereco_2 + ' ' + endereco from #endereco where ordem = 4
	select @endereco_2 = @endereco_2 + ' ' + endereco from #endereco where ordem = 5
	select @endereco_2 = @endereco_2 + ' ' + endereco from #endereco where ordem = 6
	select @endereco_2 = @endereco_2 + ' ' + endereco from #endereco where ordem = 7
	select @endereco_2 = @endereco_2 + ' ' + endereco from #endereco where ordem = 8

	select @endereco_3 = endereco from #endereco where ordem = 9
	select @endereco_3 = @endereco_3 + ' ' + endereco from #endereco where ordem = 10
	select @endereco_3 = @endereco_3 + ' ' + endereco from #endereco where ordem = 11
	select @endereco_3 = @endereco_3 + ' ' + endereco from #endereco where ordem = 12
----------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------
	...
	select @obs_2 = obs from #obs where ordem = 7
	select @obs_2 = @obs_2 + ' ' + obs from #obs where ordem = 8
	select @obs_2 = @obs_2 + ' ' + obs from #obs where ordem = 9
	select @obs_2 = @obs_2 + ' ' + obs from #obs where ordem = 10
	select @obs_2 = @obs_2 + ' ' + obs from #obs where ordem = 11
	...
	select @obs_3 = obs from #obs where ordem = 11
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 12
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 13
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 14
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 15
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 16
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 17
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 18
	select @obs_3 = @obs_3 + ' ' + obs from #obs where ordem = 19
----------------------------------------------------------------------------------------
--> Dividindo o campo endereço em duas linhas
----------------------------------------------------------------------------------------
    insert into #Hora (hora)
    select splitdata  from dbo.fncArkabSplitString(@horario, '')

	select @horario_1 = hora from #Hora where ordem = 1
	select @horario_1 = @horario_1 + ' ' + hora from #Hora where ordem = 2
	select @horario_1 = @horario_1 + ' ' + hora from #Hora where ordem = 3
	select @horario_1 = @horario_1 + ' ' + hora from #Hora where ordem = 4
	

	select @horario_2 = hora from #Hora where ordem = 5
	select @horario_2 = @horario_2 + ' ' + hora from #Hora where ordem = 6
	select @horario_2 = @horario_2 + ' ' + hora from #Hora where ordem = 7
	select @horario_2 = @horario_2 + ' ' + hora from #Hora where ordem = 8
	select @horario_2 = @horario_2 + ' ' + hora from #Hora where ordem = 9
	...
	set @script = replace(@script, '@endereco_3',isnull(@endereco_3,''))
	set @script = replace(@script, '@n_nf',isnull(@n_nf,''))
	set @script = replace(@script, '@horario_1',isnull(@horario_1,''))
	set @script = replace(@script, '@horario_2',isnull(@horario_2,''))
...
go
 exec spcCVAEtiquetaLogisticaComBorda '001','13','14387',1,'1','1',null,null,null,null,null,null
