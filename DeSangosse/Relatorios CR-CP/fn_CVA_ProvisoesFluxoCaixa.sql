IF EXISTS (SELECT * FROM sys.objects WHERE type = 'TF' AND name = 'fn_CVA_ProvisoesFluxoCaixa')
	DROP FUNCTION fn_CVA_ProvisoesFluxoCaixa
GO
CREATE FUNCTION fn_CVA_ProvisoesFluxoCaixa (@DtIni DATETIME, @DtFin DATETIME, @TpData VARCHAR(2))
	...
	BEGIN -- TRANSAÇÕES RECORRENTES
		INSERT INTO @rtnTable
			SELECT DISTINCT 
				...
				0.00,
				(T1.DocTotal * ISNULL(T5.InstPrcnt, 100))/100,
				(T1.DocTotal * ISNULL(T5.InstPrcnt, 100))/100,
				DATEADD(DAY, ISNULL(T5.InstDays, 0), T0.PlanDate),
				T1.PeyMethod
			FROM ORCL T0
		...
		BEGIN
			INSERT INTO @rtnTable
				...
				FROM ORCR T0
					...
					LEFT  JOIN OCRD T3 ON T1.AcctCode = T3.CardCode
