IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_ContasRecebidas')
	DROP PROCEDURE spc_CVA_ContasRecebidas
GO
CREATE PROCEDURE [dbo].[spc_CVA_ContasRecebidas]
(
...
select distinct 
	...
	TipoPgto,
	TabelaPgto,
	case when ValorSaldo = 0 and ValorPago = 0 then ValorDoc else ValorPago end as ValorPago,
	JurosDesconto,
