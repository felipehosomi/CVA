IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_RelatorioCP_Nova')
	DROP PROCEDURE spc_CVA_RelatorioCP_Nova
GO
...
BEGIN -- Criação de tabelas temporárias
	...
	WHERE T0.Canceled = 'N' AND 
		OA_CONTA.Conta IS NOT NULL AND
		(ISNULL(ODPO.DocDate, ISNULL(JDT1.RefDate, ISNULL(ORPC.DocDate, OPCH.DocDate))) >= @DtLancamentoIni AND ISNULL(ODPO.DocDate, ISNULL(JDT1.RefDate, ISNULL(ORPC.DocDate, OPCH.DocDate))) <= @DtLancamentoFin) AND
		(ISNULL(DPO6.DueDate, ISNULL(JDT1.DueDate, ISNULL(RPC6.DueDate, PCH6.DueDate))) >= @DtVencimentoIni AND ISNULL(DPO6.DueDate, ISNULL(JDT1.DueDate, ISNULL(RPC6.DueDate, PCH6.DueDate))) <= @DtVencimentoFin) AND
		@Tipo IN ('T', 'F')
	...
	FROM OVPM T0
		...
		LEFT JOIN OCHO T11 ON T6.CheckAbs = T11.CheckKey AND T11.Canceled = 'N' 
	OUTER APPLY 
	(
		SELECT Pagamento = 
			CASE WHEN T4.StatusTo = 'P' THEN T5.RefDate
				ELSE CASE WHEN T11.CheckKey IS NOT NULL THEN T11.CheckDate
				ELSE CASE WHEN T0.BoeAbs IS NULL AND T11.CheckKey IS NULL AND T0.DocEntry IS NOT NULL THEN T0.TaxDate
				ELSE NULL END END END
	) AS OA_DATA
	OUTER APPLY 
	(
		...
		WHERE OACT.Finanse = 'Y' AND OACT.AcctCode = 
			CASE WHEN T4.StatusTo = 'P' THEN (SELECT OPYM.GLAccount FROM OPYM WHERE OPYM.PayMethCod = T3.PayMethCod)
				ELSE CASE WHEN T11.CheckKey IS NOT NULL THEN T11.CheckAcct
				ELSE CASE WHEN T0.BoeAbs IS NULL AND T11.CheckKey IS NULL AND T0.DocEntry IS NOT NULL THEN 
	...
	WHERE T0.NoDocSum <> 0 AND T0.Canceled = 'N' --ANd OA_CONTA.Conta IS NOT NULL
		...
		AND T0.DocNum NOT IN (10964, 10987, 11094, 11146, 11193, 11218, 11268, 11434, 11596, 11630, 11699, 11782, 11788, 11837, 11997, 12054, 12082, 12123, 12159, 12163, 12251, 12322, 12386, 12441, 12492, 12577, 12585, 11428, 11429, 12001, 12002, 12117, 12118, 12268, 12269, 12639, 13268)
		AND
