...
namespace ServiceLayerHelper
{
    ...
    public class SLConnection
    {
        ...
        public async Task Login(bool forceLogin = false)
        {
            ...
            {
                cookieJar.AddOrReplace(new FlurlCookie(cookie.Name, cookie.Value, cookie.OriginUrl) { Secure = true, HttpOnly = cookie.HttpOnly, Path = "/b1s" });
            }
            ...
            uint? top = null, string skip = null, string apply = null, string expand = null)
        {
            ...
            {
                var getRequest = BuildGetRequest(resource, filter, select, orderBy, top, skip, apply, expand).WithHeader("B1S-CaseInsensitive", "true");
        ...
        public async Task<T> PatchAsync<T>(string resource, T data)
        {
            return await ExecuteRequest(async () =>
            {
                var result = await ServiceLayerRootUrl
        ...
        public async Task<IFlurlResponse> PostBatchAsync(IEnumerable<BatchRequest> requests)
        {
            return await ExecuteRequest(async () =>
            {
        ...
        public async Task<IFlurlResponse> PostBatchInSingleChangesetAsync(IEnumerable<BatchRequest> requests)
        {
            return await ExecuteRequest(async () =>
            {
                if (requests == null || requests.Count() == 0)
        ...
        private async Task<T> ExecuteRequest<T>(Func<Task<T>> action)
        {
            ...
            while (true)
            {
                ...
                catch (FlurlHttpException ex)
                {
                    ...
                    if (!returnCodesToRetry.Any(x => (int)x == ex.Call.Response.StatusCode))
                    {
                        break;
                    }
