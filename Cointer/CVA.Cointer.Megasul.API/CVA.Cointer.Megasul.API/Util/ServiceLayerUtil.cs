...
namespace SBO.Hub.SBOHelpers
{
    public class ServiceLayer
    {
        ...
        public string Cancel(string entity, int docEntry)
        {
            return Task.Run(async () =>
            {
                return await CancelAsync(entity, docEntry);
            }).Result;
        }
        ...
        public async Task<string> CancelAsync(string entity, int docEntry)
        {
            try
            {
                if (AutoLogin && DateTime.Now > SessionTimeOut)
                ...
                Uri uri = new Uri($"{ApiUrl}{entity}({docEntry})/Cancel");
        ...
        public async Task<string> PostAsync<T>(string entity, T item, bool tryAgain = true) where T : class
        {
            try
            {
                if (AutoLogin && DateTime.Now > SessionTimeOut)
                ...
                else
                {
                    ...
                    if (String.IsNullOrEmpty(error) && tryAgain)
                    {
                        SessionTimeOut = DateTime.Now.AddMinutes(-1);
                        return await PostAsync(entity, item, false);
                    }
        ...
        private string HandleError(string response)
        {
            ...
            {
                return "Sessão encerrada, por favor tente novamente";
            }
