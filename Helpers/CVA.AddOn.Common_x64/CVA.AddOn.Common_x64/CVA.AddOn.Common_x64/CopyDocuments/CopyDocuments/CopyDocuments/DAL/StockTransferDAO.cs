...
namespace CopyDocuments.DAL
{
    public class StockTransferDAO
    {
        ...
        public void DoCopy()
        {
            Console.WriteLine($"Buscando Transferências de Estoque");
            ...
            rstDocFrom.DoQuery("SELECT DocEntry FROM OWTR WHERE ISNULL(U_CVA_Imported, 0) = 0");
            ...
            while (!rstDocFrom.EoF)
            {
                ...
                for (int i = 0; i < stockFrom.Lines.Count; i++)
                {
                    ...
                    if (stockFrom.Lines.SerialNumbers.Quantity > 0)
                    {
                        ...
                        {
                            if (!String.IsNullOrEmpty(stockTo.Lines.SerialNumbers.InternalSerialNumber))
                            {
                                stockTo.Lines.SerialNumbers.Add();
                            }

                            stockFrom.Lines.SerialNumbers.SetCurrentLine(j);
                            stockTo.Lines.SerialNumbers.InternalSerialNumber = stockFrom.Lines.SerialNumbers.InternalSerialNumber;
                ...
                if (stockTo.Add() != 0)
                {
                    Console.WriteLine($"Transferência estoque: {stockFrom.DocEntry} - {ConnectionTo.oCompany.GetLastErrorDescription()}");
                }
                else
                {
