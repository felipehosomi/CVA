...
namespace CVA.View.Comissionamento.Controller
{
    public class AppController
    {
        ...
        private void VerifyData()
        {
            try
            {
                ...
                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "FABRICANTE"))
                    UserFields.Create("CVA_REGR_COMISSAO", "FABRICANTE", "% de Comissão", 10, SAPbobsCOM.BoFieldTypes.db_Numeric);
                ...
                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "COMISSAO_REAL"))
                    UserFields.Create("CVA_REGR_COMISSAO", "COMISSAO_REAL", "Comissão real", 10, SAPbobsCOM.BoFieldTypes.db_Float, SAPbobsCOM.BoFldSubTypes.st_Percentage);
                ...
                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "GROUP"))
                    UserFields.Create("CVA_REGR_COMISSAO", "GROUP", "Grupo de cliente", 10, SAPbobsCOM.BoFieldTypes.db_Numeric);
                ...
                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "COMNAME"))
                    UserFields.Create("CVA_REGR_COMISSAO", "COMNAME", "Nome do comissionado", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "VNDNAME"))
                    UserFields.Create("CVA_REGR_COMISSAO", "VNDNAME", "Nome do vendedor", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "ITEMNAME"))
                    UserFields.Create("CVA_REGR_COMISSAO", "ITEMNAME", "Nome do item", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "ITMSGRPNAM"))
                    UserFields.Create("CVA_REGR_COMISSAO", "ITMSGRPNAM", "Nome do grupo do item", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "PRCNAME"))
                    UserFields.Create("CVA_REGR_COMISSAO", "PRCNAME", "Nome do centro de custo", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "CARDNAME"))
                    UserFields.Create("CVA_REGR_COMISSAO", "CARDNAME", "Nome do cliente", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "COUNTY"))
                    UserFields.Create("CVA_REGR_COMISSAO", "COUNTY", "Nome da cidade", 254);

                if (!UserFields.Exists("@CVA_REGR_COMISSAO", "GROUPNAME"))
                    UserFields.Create("CVA_REGR_COMISSAO", "GROUPNAME", "Nome do grupo de cliente", 254);
                #endregion
                ...
                UserQueries.AssignFormattedSearch("REGRGROUP", Properties.Resources.REGRGROUP, "UDOREGR", "U_GROUP");
                #endregion
        ...
        {
            Menus.Add("43520", "CVA", "CVA - Comissionamento", -1, SAPbouiCOM.BoMenuType.mt_POPUP, $"{AppDomain.CurrentDomain.BaseDirectory}\\Files\\if_bank-building_532649.bmp");
            ...
            Menus.Add("CVA", "UPDT", "Atualização de regras", 7, SAPbouiCOM.BoMenuType.mt_STRING);
        }
        ...
        private void SetFilters()
        {
            ...
            Filters.Add("UPDT", SAPbouiCOM.BoEventTypes.et_MENU_CLICK);
            ...
            //Filters.Add(View.RegraComissaoForm.Type, SAPbouiCOM.BoEventTypes.et_UDO_FORM_BUILD);
            //Filters.Add(View.RegraComissaoForm.Type, SAPbouiCOM.BoEventTypes.et_UDO_FORM_OPEN);            
        ...
        private void Application_MenuEvent(ref SAPbouiCOM.MenuEvent pVal, out bool bubbleEvent)
        {
            ...
            try
            {
                ...
                if (pVal.MenuUID.Equals("UPDT") && !pVal.BeforeAction)
                {
                    ...
                    {
                        Helpers.DIHelper.AtualizaRegras();
                ...
                if (pVal.MenuUID.Equals("1287") && !pVal.BeforeAction)
                {
        ...
        private void Application_ItemEvent(string FormUID, ref SAPbouiCOM.ItemEvent pVal, out bool bubbleEvent)
        {
            ...
            try
            {
                ...
                //if (pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST) && pVal.ItemUID.In(View.RegraComissaoForm.EditCodigoItem, View.RegraComissaoForm.EditCentroCusto, View.RegraComissaoForm.EditCliente))
                //{
                ...
                //    }
                //}
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCliente) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_PICKER_CLICKED) && pVal.BeforeAction)
                {
                    pickerClicked = true;
                }

                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCliente) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    var oForm = Application.Forms.ActiveForm;
                    ...
                    if (oEdit.Value != null)
                    {
                        ...
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery($"SELECT CardName FROM OCRD WHERE CardCode = '{oEdit.Value.ToString()}'");
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditClienteDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                            //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceCardName).ValueEx = oRecordset.Fields.Item(0).Value.ToString();
                        }
                        else
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditClienteDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCentroCusto) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_PICKER_CLICKED) && pVal.BeforeAction)
                {
                    pickerClicked = true;
                }

                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCentroCusto) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    var oForm = Application.Forms.ActiveForm;
                    ...
                    if (oEdit.Value != null)
                    {
                        ...
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery($"SELECT PrcName FROM OPRC WHERE PrcCode = '{oEdit.Value.ToString()}'");
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditCentroCustoDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                            //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourcePrcName).ValueEx = oRecordset.Fields.Item(0).Value.ToString();
                        }
                        else
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditCentroCustoDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCodigoItem) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_PICKER_CLICKED) && pVal.BeforeAction)
                {
                    pickerClicked = true;
                }

                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCodigoItem) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    var oForm = Application.Forms.ActiveForm;
                    ...
                    if (oEdit.Value != null)
                    {
                        ...
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery($"SELECT ItemName FROM OITM WHERE ItemCode = '{oEdit.Value.ToString()}'");
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditCodigoItemDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                            //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceItemName).ValueEx = oRecordset.Fields.Item(0).Value.ToString();
                        }
                        else
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditCodigoItemDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditComissionado) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    ...
                    if (oEdit.Value != null)
                    {
                        if (!string.IsNullOrEmpty(oEdit.Value.ToString()))
                        {
                            ...
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditComissionadoDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                        ...
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditComissionadoDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditGroupCode) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_PICKER_CLICKED) && pVal.BeforeAction)
                {
                    pickerClicked = true;
                }

                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditGroupCode) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    var oForm = Application.Forms.ActiveForm;
                    var oEdit = (SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGroupCode).Specific;
                    ...
                    if (oEdit.Value != null)
                    {
                        ...
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery($"SELECT GroupName FROM OCRG WHERE GroupCode = {oEdit.Value.ToString()}");
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGroupName).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                            //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceGroupName).ValueEx = oRecordset.Fields.Item(0).Value.ToString();
                        }
                        else
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGroupName).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditVendedor) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    ...
                    if (oEdit.Value != null)
                    {
                        if (!string.IsNullOrEmpty(oEdit.Value.ToString()))
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery($"SELECT SlpName FROM OSLP WHERE SlpCode = {oEdit.Value.ToString()}");
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditVendedorDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                        ...
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditVendedorDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditGrupoItem) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    ...
                    if (oEdit.Value != null)
                    {
                        if (!string.IsNullOrEmpty(oEdit.Value.ToString()))
                        {
                            ...
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGrupoItemDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                        ...
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGrupoItemDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCidade) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE) && !pVal.BeforeAction && pickerClicked)
                {
                    ...
                    if (oEdit.Value != null)
                    {
                        if (!string.IsNullOrEmpty(oEdit.Value.ToString()))
                        {
                            ...
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditCidadeDesc).Specific).Value = oRecordset.Fields.Item(0).Value.ToString();
                        ...
                        {
                            ((SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditCidadeDesc).Specific).Value = string.Empty;
                        }
                    }
                    else
                    {
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditGroupCode) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_LOST_FOCUS) && !pVal.BeforeAction)
                {
                    var oForm = Application.Forms.ActiveForm;
                    var oEdit = (SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGroupCode).Specific;
                    ...
                    if (oEdit.Value != null)
                    {
                        ...
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery("SELECT Code FROM [@CVA_CRIT_COMISSAO] WHERE Name LIKE '%Grupo de cliente%'");
                ...
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.ItemUID.Equals(View.RegraComissaoForm.EditCliente) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_LOST_FOCUS) && !pVal.BeforeAction)
                {
                    ...
                    if (oEdit.Value != null)
                    {
                        ...
                        {
                            var oRecordset = (SAPbobsCOM.Recordset)Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                            oRecordset.DoQuery("SELECT Code FROM [@CVA_CRIT_COMISSAO] WHERE Name = 'Cliente'");
                ...
                if (pVal.FormTypeEx == View.RegraComissaoForm.Type && pVal.ItemUID == View.RegraComissaoForm.ButtonOk && pVal.EventType == SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED && pVal.BeforeAction == false && pVal.ActionSuccess == true && pVal.FormMode == 3)
                {
                    if (!pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_VALIDATE))
                    {
                        ...
                        //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceSlpName1).ValueEx = null;
                        //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceSlpName2).ValueEx = null;
                        //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceGroupName).ValueEx = null;
                        Application.ActivateMenuItem("1281"); 
                    }
                }

                if (pVal.FormMode.In((int)SAPbouiCOM.BoFormMode.fm_ADD_MODE, (int)SAPbouiCOM.BoFormMode.fm_UPDATE_MODE))
                {
                ...
                if (pVal.FormTypeEx.Equals(View.CalculoComissaoForm.Type) && pVal.ItemUID.Equals(View.CalculoComissaoForm.ButtonRecalcular) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) && !pVal.BeforeAction)
                {
                    ...
                    foreach (DataRow dr in data.Rows)
                    {
                        ...
                        udColDataDoDocumento.Value = DateTime.Parse(dr["DocDate"].ToString()).Date.ToString("yyyyMMdd");
                        udColDataDoVencimento.Value = DateTime.Parse(dr["DueDate"].ToString()).Date.ToString("yyyyMMdd");
                        doctry = dr["DocEntry"].ToString();
                        udColChaveDoDocumento.Value = Convert.ToInt32(dr["DocEntry"]).ToString();
            ...
            {
               // Application.MessageBox(ex.ToString() + " Este aqui : "+ doctry);
                Application.SetStatusBarMessage(ex.Message);
                ret = false;
            }
        ...
        private void Application_FormDataEvent(ref SAPbouiCOM.BusinessObjectInfo BusinessObjectInfo, out bool bubbleEvent)
        {
            ...
            try
            {
                if (pVal.FormTypeEx.Equals(View.RegraComissaoForm.Type) && pVal.EventType.Equals(SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD) && pVal.BeforeAction)
                {
                    ...
                    var editGroupCode = (SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGroupCode).Specific;
                    ...
                    else
                    {
                        if (comboEstado != null && comboEstado.Selected != null && !string.IsNullOrEmpty(comboEstado.Selected.Value.ToString()))
                        {
                            ...
                            oForm.DataSources.DBDataSources.Item("@CVA_REGR_COMISSAO").SetValue("U_COUNTY", 0, "");
                    ...
                    {
                        if (editGroupCode != null && editGroupCode.Value != null && !string.IsNullOrEmpty(editGroupCode.Value.ToString()))
                        {
                            grupoCliente = int.Parse(editGroupCode.Value.ToString());
                            oForm.DataSources.DBDataSources.Item("@CVA_REGR_COMISSAO").SetValue("U_CLIENTE", 0, "");
                            oForm.DataSources.DBDataSources.Item("@CVA_REGR_COMISSAO").SetValue("U_CARDNAME", 0, "");
                            //oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceCardName).ValueEx = "";
                        }
                    }
                    ...
                    if (editCodigoItem != null && editCodigoItem.Value != null && !string.IsNullOrEmpty(editCodigoItem.Value.ToString()))
                    {
                        ...
                        oForm.DataSources.DBDataSources.Item("@CVA_REGR_COMISSAO").SetValue("U_ITMSGRPNAM", 0, "");
                    ...
                    else
                    {
                        if (editGrupoItem != null && editGrupoItem.Value != null && !string.IsNullOrEmpty(editGrupoItem.Value.ToString()))
                        {
                            ...
                            oForm.DataSources.DBDataSources.Item("@CVA_REGR_COMISSAO").SetValue("U_ITEMNAME", 0, "");
                ...
                //    oEdit = (SAPbouiCOM.EditText)oForm.Items.Item(View.RegraComissaoForm.EditGroupCode).Specific;
                //    if (oEdit != null && oEdit.Value != null && !string.IsNullOrEmpty(oEdit.Value.ToString()))
                //    {
                //        oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceGroupName).ValueEx = DIHelper.GetGroupName(oEdit.Value.ToString());
                //    }
                ...
                //    oForm.DataSources.UserDataSources.Item(View.RegraComissaoForm.UserDataSourceGroupName).ValueEx = string.Empty;
                //    oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE;
                //}
            }
            catch (Exception ex)
