...
namespace CVA.View.Comissionamento.Controller
{
    public class ComissoesController
    {
        ...
        public static string SetInvoices(string dataInicial, string dataFinal, bool todas, bool pagas, bool naoPagas)
        {
            ...
            if (oRecordset.RecordCount > 0)
            {
                while (!oRecordset.EoF)
                {
                    ...
                    var taxDate = oRecordset.Fields.Item("U_TAXDATE").Value.ToString();
                    ...
                    if (!VerifyUdo(docEntry, objType, linha, itemCode, docEntryNF, instId, docEntryCR))
                    AddUdo(prioridade, comissionado, cardCode, cardName, regra, docDate, dueDate, objType, docEntry, linha, itemCode, itemName, ocrCode, instId, total, taxSum, percentComissao, valorComissao, comissaoPaga, dataPagamento, taxDate, docEntryNF,docEntryCR);
                    oRecordset.MoveNext();
                }
        ...
        private static void AddUdo(string prioridade, string comissionado, string cardCode, string cardName, string regra, string docDate, string dueDate, string objType, string docEntry, string linha, string itemCode, string itemName, string ocrCode, string instId, string total, string taxSum, string percentComissao, string valorComissao, string comissaoPaga, string dataPagamento, string taxDate, string docEntryNF, string docEntryCR)
        {
            ...
            oGeneralData.SetProperty("U_TAXDATE", taxDate);
        ...
        private static bool VerifyUdo(string docEntry, string objType, string linha, string itemCode, string docEntryNF, string instId, string docEntryCR)
        {
            bool verifyUDO = (System.Configuration.ConfigurationManager.AppSettings["verifyUDO"] == "Y");
            var ret = false;
            if (verifyUDO)
            {
                var oRecordset = (Recordset)B1Connection.Instance.Company.GetBusinessObject(BoObjectTypes.BoRecordset);
                oRecordset.DoQuery($"SELECT 1 FROM [@CVA_CALC_COMISSAO] WHERE U_DOCENTRY = {docEntry} AND U_OBJTYPE = {objType} AND U_LINENUM = {linha} and U_ITEMCODE='{itemCode}' and U_DOCENTRY_NF={docEntryNF} and U_PARCELA={instId} and U_DOCENTRY_CR={docEntryCR}");
