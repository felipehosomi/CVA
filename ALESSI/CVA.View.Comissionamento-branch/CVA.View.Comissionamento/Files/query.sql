/****** Object:  StoredProcedure [dbo].[spc_CVA_PagarComissao]    Script Date: 21/06/2017 09:10:58 ******/
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_PagarComissao')
	DROP PROCEDURE [dbo].[spc_CVA_PagarComissao]
GO
/****** Object:  StoredProcedure [dbo].[spc_CVA_Comissoes]    Script Date: 21/06/2017 09:10:58 ******/
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spc_CVA_Comissoes')
	DROP PROCEDURE [dbo].[spc_CVA_Comissoes]
GO
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'FN' AND name = 'fn_CVA_PositionCriteria')
	DROP FUNCTION [dbo].[fn_CVA_PositionCriteria]
GO
USE [###SB0TESTE_COMISSAO###]
GO
/****** Object:  StoredProcedure [dbo].[spc_CVA_Comissoes]    Script Date: 10/06/2020 16:35:32 ******/
SET ANSI_NULLS ON
GO
...
--declare @Status char(1) = 'T', @View char(1) = 'N', @DataInicial datetime, @DataFinal datetime
--SET @DataInicial = '2019/01/01'
--SET @DataFinal= '2019/01/31'
...
--EXEC [spc_CVA_Comissao_Pedidos] @DataInicial, @DataFinal
...
SELECT DISTINCT
	...
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN OA_TOTAL.LineTotal * (T2.InstPrcnt/100) * ((((T14.SumApplied) * 100)/ T2.InsTotal)/100) ELSE 0 END AS LineTotal_Min_IMPADIC
...
--	LEFT  JOIN OSLP T7 WITH(NOLOCK) ON T3.SlpCode = T7.SlpCode
	...
	INNER JOIN ORCT T15 WITH(NOLOCK) ON T14.DocNum = T15.DocEntry and T15.Canceled='N'
...
SELECT DISTINCT
	...
	, CASE WHEN ISNULL(T6.U_CVA_IMPADC, 'N') = 'N' AND ISNULL(T6.U_CVA_IMPINCL, 'N') = 'Y' THEN (T23.DrawnSum/(OA_MAX.TotalItens + t24.VatSum)) * T25.LineTotal ELSE 0 END AS LineTotal_Min_IMPADIC
...
WHERE T0.CANCELED = 'N' And T24.CANCELED = 'N' AND OA_DATA.DataPagamento IS NOT NULL AND OA_DATA.DataPagamento >= @DataInicial AND OA_DATA.DataPagamento <= @DataFinal
AND (
	SELECT COUNT(OACT.AcctCode) FROM JDT1 WITH(NOLOCK)
	INNER JOIN OACT WITH(NOLOCK) ON JDT1.Account = OACT.AcctCode AND OACT.Finanse = 'Y'
	WHERE JDT1.TransId = T15.TransId AND JDT1.TransType = T15.ObjType
...
--LEFT JOIN #tbl_docs_recebimento T1 WITH(NOLOCK) ON T0.DocEntry = T1.DocEntry AND T0.ObjType = T1.ObjType AND T0.InstlmntID = T1.InstlmntID
--WHERE T1.DocEntry IS NULL
UNION ALL
SELECT DISTINCT *, 0 , 0 FROM #tbl_docs_pedido T0
UNION ALL
SELECT DISTINCT * FROM #tbl_docs_recebimento T0
ORDER BY 1, 6, 2, 11, 8

--select sum(LineTotal_Min_IMPADIC) from #tbl_docs_aux
--where DocEntry = 4247

--select * from #tbl_docs_aux Order BY Serial
...
BEGIN
	...
	if isnull(@Setor, 0) = -1 set @Setor = 0
...
;with cte as (
	select *
		, RN = row_number() over (partition by Comissionado, Momento, ObjType, DocEntry, InstlmntID, LineNum, DocEntryNF, DocEntryCR order by Prioridade)
...
insert into #tmp_table_ret
	select 
		...
		T0.U_DOCENTRY_NF, 0 as U_DOCENTRY_CR
	from [@CVA_CALC_COMISSAO] T0 WITH(NOLOCK)
		inner join [@CVA_REGR_COMISSAO] T1 WITH(NOLOCK) ON T0.U_REGRA = T1.Code AND T1.U_ATIVO = 'Y'
		inner join [@CVA_CRIT_COMISSAO] T2 WITH(NOLOCK) ON T1.U_PRIORIDADE = T2.Code
	where T0.U_PAGO = 'Y'
...
INSERT INTO #tmp_table_ret
	...
	FROM #tbl_docs_fat T0
		LEFT JOIN #tmp_table_ret T1 WITH(NOLOCK) ON T0.DocEntry = T1.U_DOCENTRY AND T0.ObjType = T1.U_OBJTYPE AND T0.LineNum = T1.U_LINENUM AND T0.InstlmntID = T1.U_PARCELA AND T0.RegrCode = T1.U_REGRA
	...
	begin
		select distinct
			...
			U_VALOR,
			U_PARCELA,
			CAST(U_IMPOSTOS AS decimal (6,2)) AS U_IMPOSTOS,
			U_COMISSAO,
			CAST(U_VALORCOMISSAO AS decimal (6,2)) AS U_VALORCOMISSAO,
	...
	begin
		select distinct 
			...
			null as U_ITEMNAME,
			null as U_LINENUM,
			SUM(U_VALOR) AS U_VALOR,
			U_PARCELA,
			CAST (SUM(U_IMPOSTOS) AS decimal (6,2)) AS U_IMPOSTOS,
			AVG(U_COMISSAO) AS U_COMISSAO,
			CAST (SUM(U_VALORCOMISSAO) AS decimal (6,2)) AS U_VALORCOMISSAO,
			ISNULL(U_TAXDATE, '1900-01-01') AS U_TAXDATE,
