...
/****** Object:  StoredProcedure [dbo].[spcNMSContasRecebidasPorCliente]    Script Date: 06/09/2015 09:17:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spcNMSContasRecebidasPorCliente]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spcNMSContasRecebidasPorCliente]
GO
...
CREATE PROC [dbo].[spcNMSContasRecebidasPorCliente] (
	@cardcode varchar(30),
	@dateini date,
	@datefim date,
	@tpData varchar(2)                               
...
insert into #ret (operacao, objtype, metodo, nfserial, nfentry, parcela, crentry, emissao, Codigo, Parceiro, valordocumento 
                , valorparcela, vencimento, liquidacao, valorpago, transacao, jrnlmemo, transid, boenum, boemeth,Juros)
 ...
 declare cp1 cursor local fast_forward read_only for
   select operacao, objtype, metodo, crentry, parcela, vencimento, liquidacao, sum(valorparcela), codigo, parceiro, valorpago, transacao, jrnlmemo, transid, boenum, boemeth,Juros
     from #ret
    group by operacao, objtype, metodo, crentry, parcela, vencimento, liquidacao, codigo, parceiro, valorpago, transacao, jrnlmemo, transid, boenum, boemeth,Juros

 open cp1
 while 1 = 1
 begin
  fetch next from cp1 into @voperacao, @vobjtype, @vmetodo, @vcrentry, @vparcela, @vvencimento, @vliquidacao, @vvalorparcela, @vcodigo, @vparceiro, @vvalorpago, @vtransacao, @vjrnlmemo, @vtransid, @vboenum, @vboemeth,@Juros
  ...
  insert into #o (operacao, objtype, metodo, parcela, crentry, Codigo, parceiro, valorparcela
    ...
    select @voperacao, @vobjtype, @vmetodo, @vparcela, @vcrentry, @vCodigo, @vparceiro, @vvalorparcela
         ...
         , @vboemeth, @notas,coalesce(@Juros,0)
