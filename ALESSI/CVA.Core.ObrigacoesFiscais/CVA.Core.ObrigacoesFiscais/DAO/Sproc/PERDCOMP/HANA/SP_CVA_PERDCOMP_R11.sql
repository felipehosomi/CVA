CREATE PROCEDURE  "SP_CVA_PERDCOMP_R11"   
 (
	  in Filial   INT  
	 ,in DataDe   DATE  
	 ,in DataAte  DATE
...
INSERT INTO Docs2  
SELECT   
  CAST(YEAR("DocDate") AS VARCHAR(4)),  
  CASE WHEN LENGTH(CAST(MONTH("DocDate") AS VARCHAR(4))) > 2 THEN CAST(MONTH("DocDate") AS VARCHAR(4)) ELSE   
      LPAD('0', 2 - LENGTH(CAST(MONTH("DocDate") AS VARCHAR(4)))) + RTrim(CAST(MONTH("DocDate") AS VARCHAR(4))) END,   
       "CFOPCode",  
   SUM("IPI_BaseSum"),  
   SUM("IPI_TaxSum"),  
   SUM("IPI_U_ExcAmtL"),  
   SUM("IPI_U_OthAmtL")  
  FROM  "CVA_LIVRODEENTRADA_ITENS"   
 WHERE "BPLId" = Filial AND "DocDate" BETWEEN DataDe AND DataAte   
 GROUP BY CAST(YEAR("DocDate") AS VARCHAR(4)), CAST(MONTH("DocDate") AS VARCHAR(4)), "CFOPCode"
 ORDER BY "CFOPCode";
