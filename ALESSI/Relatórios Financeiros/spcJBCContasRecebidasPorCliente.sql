...
insert into #ret (operacao, objtype, metodo, nfserial, nfentry, parcela, crentry, emissao, Codigo, Parceiro, valordocumento 
  ...
  select 'ADTO - Boleto bancário' as 'Operacao'
   ...
   inner join OCRD
      on odpi.CardCode = ocrd.CardCode  and (ocrd.U_CVA_GRUPOECON=@CVA_GRUPOECON or @CVA_GRUPOECON='*')
   where orct.Canceled = 'N'
     and oboe.BoeStatus = 'P'
     and orct.BoeSum > 0
     and odpi.CardCode = ISNULL(@cardcode, odpi.cardcode)
	 and (odpi.slpcode=@slpCode or @slpCode=0)
	 and 
	 (
		 ((oboe.DueDate between ISNULL(@dateini, oboe.DueDate) and ISNULL(@datefim, oboe.DueDate))  and @tpData='V')
		or
		 ((oboe.PmntDate between ISNULL(@dateini, oboe.PmntDate) and ISNULL(@datefim, oboe.PmntDate))and @tpData='L' )
	 )
  ...
  select 'ADTO - Transferência bancária' as 'operacao'
   ...
   where orct.Canceled = 'N'
	 ...
	 and (odpi.slpcode=@slpCode or @slpCode=0)
	 and (
		 ((orct.DocDueDate between ISNULL(@dateini, orct.DocDueDate) and ISNULL(@datefim, orct.DocDueDate))  and @tpData='V')
		or
		 ((orct.TrsfrDate between ISNULL(@dateini, orct.TrsfrDate) and ISNULL(@datefim, orct.TrsfrDate))and @tpData='L' )
	 )
     --and isnull(odpi.Serial, 0) = ISNULL(@serial, isnull(odpi.serial, 0))
  ...
  select 'ADTO - Dinheiro' as 'operacao'
   ...
   where orct.Canceled = 'N'
	 ...
	 and (
		 ((orct.DocDueDate between ISNULL(@dateini, orct.DocDueDate) and ISNULL(@datefim, orct.DocDueDate))  and @tpData='V')
		or
		 ((orct.DocDate between ISNULL(@dateini, orct.DocDate) and ISNULL(@datefim, orct.DocDate))and @tpData='L' )
	 )
     --and isnull(odpi.Serial, 0) = ISNULL(@serial, isnull(odpi.serial, 0))
     --and orct.DocDueDate between ISNULL(@duedateini, orct.DocDueDate) and ISNULL(@duedatefim, orct.DocDueDate)
  ...
  select 'NF - Boleto bancário' as 'operacao'
   ...
   inner join OBOE
      on orct.BoeNum = oboe.BoeNum
   inner join OCRD
      on oinv.CardCode = ocrd.CardCode and (ocrd.U_CVA_GRUPOECON=@CVA_GRUPOECON or @CVA_GRUPOECON='*')
   where orct.Canceled = 'N'
     and oboe.BoeStatus = 'P'
     and orct.BoeSum > 0
     and oinv.CardCode = ISNULL(@cardcode, oinv.cardcode)
	 and (OINV.slpcode=@slpCode or @slpCode=0)
	 and (
		 ((oboe.DueDate between ISNULL(@dateini, oboe.DueDate) and ISNULL(@datefim, oboe.DueDate))  and @tpData='V')
		or
		 (((SELECT 
		...
		WHERE 
			T0.[BOENumber] = oboe.BoeNum  
			and T1.[StatusTo]='P') between ISNULL(@dateini, (SELECT 
			max(T1.[TranDate] )
		FROM  
			[dbo].[BOT1] T0  
			INNER  JOIN [dbo].[OBOT] T1  ON  T1.[AbsEntry] = T0.[AbsEntry]   
		WHERE 
			T0.[BOENumber] = oboe.BoeNum  
			and T1.[StatusTo]='P')) and ISNULL(@datefim, (SELECT 
			max(T1.[TranDate] )
		FROM  
			[dbo].[BOT1] T0  
			INNER  JOIN [dbo].[OBOT] T1  ON  T1.[AbsEntry] = T0.[AbsEntry]   
		WHERE 
			T0.[BOENumber] = oboe.BoeNum  
			and T1.[StatusTo]='P')))and @tpData='L' )
	 )
  ...
  select 'NF - Transferência bancária' as 'operacao'
   ...
   where orct.Canceled = 'N'
	 ...
	 and (OINV.slpcode=@slpCode or @slpCode=0)
	 and (
		 (((select INV6.DueDate from INV6 where INV6.DocEntry = oinv.DocEntry and INV6.InstlmntID=rct2.InstId ) between ISNULL(@dateini, (select INV6.DueDate from INV6 where INV6.DocEntry = oinv.DocEntry and INV6.InstlmntID=rct2.InstId )) and ISNULL(@datefim, (select INV6.DueDate from INV6 where INV6.DocEntry = oinv.DocEntry and INV6.InstlmntID=rct2.InstId )))  and @tpData='V')
		or
		 ((orct.TrsfrDate between ISNULL(@dateini,orct.TrsfrDate) and ISNULL(@datefim, orct.TrsfrDate))and @tpData='L' )
	 )
     --and isnull(oinv.Serial, 0) = ISNULL(@serial, isnull(oinv.serial, 0))
  ...
  select distinct 
   ...
   inner join OBOE
      on orct.BoeNum = oboe.BoeNum
   inner join OCRD
      on jdt1.ShortName = OCRD.CardCode and (ocrd.U_CVA_GRUPOECON=@CVA_GRUPOECON or @CVA_GRUPOECON='*')
   where orct.Canceled = 'N'
     and oboe.BoeStatus = 'P'
     and orct.BoeSum > 0
...
select *,DATEDIFF ( day , Vencimento , liquidacao ) as Dias from #o
where  
...
execute spcJBCContasRecebidasPorCliente 'C00003177','2015-01-01','2018-02-28','V','*',0