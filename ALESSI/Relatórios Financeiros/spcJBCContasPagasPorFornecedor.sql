...
where	
	...
	or
	(ISNULL(@dataIni 	, liquidacao)<>ISNULL(@dataFim , liquidacao) and @tpData<>'L' and @tpData<>'V' and @tpData<>'LC')--opção invalida não traz nada
	or (@tpData='LC' and (tb.Lançamento between ISNULL(@dataIni, Lançamento) and ISNULL(@dataFim 	, Lançamento)) )
	)
