...
/****** Object:  StoredProcedure [dbo].[SpcCVAFluxoCaixa_CARGA]    Script Date: 06/09/2015 09:17:57 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SpcCVAFluxoCaixa_CARGA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SpcCVAFluxoCaixa_CARGA]
GO
...
CREATE proc [dbo].[SpcCVAFluxoCaixa_CARGA]
...
EXECUTE [SpcCVAContasAReceberPorVencimento] 
   '*'
  --,'1900-11-20'
  --,'2050-11-20'
  ,@dtInicial
...
execute SpcCVAContasAPagarPorVencimento 
   '*'
  --,'1900-11-20'
  --,'2050-11-20'
  ,@dtInicial
 ...
 --   select 
	...
	where origem not like '%Recorrente%'
  ...
  insert into #lancamentos (tipo, dt_vencimento, valor)
	...
	from 
		#ContasAReceberPorVencimento   
	where transtype not like '%Recorrente%' 
